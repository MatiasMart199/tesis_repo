...
// Función principal para listar datos (CORREGIDA)
function listar(fecha_inicio = null, fecha_fin = null) {
    return new Promise((resolve, reject) => {
        let datosEnvio = {};
        ...
        if (fecha_inicio && fecha_fin) {
            if (fecha_inicio > fecha_fin) {
                mostrarAlerta("warning", "La fecha de inicio no puede ser mayor a la fecha fin.");
                reject("Fechas inválidas");
                return;
            }
            datosEnvio = { fecha_inicio, fecha_fin };
        }
...
function mostrarDatosEnTabla(datos) {
    ...
    if (!datos || datos.length === 0) {
        ...
        datos.forEach(rs => {
            lineas += `
                <tr>
                    <td>${escapeHtml(rs.id_cob)}</td>
                    <td>${escapeHtml(rs.vc_nro_factura)}</td>
                    <td>${escapeHtml(rs.fecha)}</td>
                    <td>${escapeHtml(rs.total_efectivo)}</td>
                    <td>${escapeHtml(rs.total_cheque)}</td>
                    <td>${escapeHtml(rs.total_tarjeta)}</td>
                    <td>${escapeHtml(rs.total_transfe)}</td>
                    <td>${escapeHtml(rs.total_general)}</td>
                </tr>
            `;
        });
    }
...
function btnFiltrar() {
    ...
    // Validaciones
    if (!fecha_inicio || !fecha_fin) {
        mostrarAlerta("warning", "Debe seleccionar ambas fechas.");
        return;
    }
    ...
    console.log("🔍 Aplicando filtro:", { fecha_inicio, fecha_fin });
    
    // Mostrar loading
    mostrarAlerta("info", "Aplicando filtro...", true);

    listar(fecha_inicio, fecha_fin).then(() => {
...
function prepararDatosParaGrafico(datos) {
    ...
    datos.forEach(item => {
        ...
        const montoLimpio = item.total_general.toString()
