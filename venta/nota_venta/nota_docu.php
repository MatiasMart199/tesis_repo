...
// Determinar tipo de documento y configuraciones
$tipoNota = strtoupper($cabecera[0]['not_tipo_nota']);
$esCredito = ($tipoNota == 'CREDITO' || $tipoNota == 'CRÉDITO');
$esDebito = ($tipoNota == 'DEBITO' || $tipoNota == 'DÉBITO');

// Configuraciones según tipo de nota
$config = [
    'color' => $esCredito ? '#e6f7ff' : ($esDebito ? '#fff0f0' : '#f0f0f0'),
    'leyenda' => $esCredito ? 
        '"Documento que corrige valores a favor del cliente"' : 
        ($esDebito ? 
            '"Documento que corrige valores a favor del emisor"' : 
            '"Documento comercial"'),
    'titulo' => 'NOTA DE ' . $tipoNota
];
...
$pdf->SetTitle($config['titulo'] . ' - ' . $cabecera[0]['not_nro_documento']);
$pdf->SetSubject('Nota de ' . $tipoNota);
$pdf->SetKeywords('NOTA, TCPDF, PDF');
...
// Para notas de crédito, mostrar valores negativos
$signoTotal = ($esCredito) ? -1 : 1;
$colorTotal = ($esCredito) ? 'style="color: red;"' : '';
...
<table width="100%" cellpadding="3" style="font-size: 10px;">
    ...
    <tr>
        ...
        <td width="30%" class="text-center bordered" style="background-color: ' . $config['color'] . ';">
            <div class="large bold">' . $config['titulo'] . '</div>
...
// Sección de motivo y referencia para notas de crédito/débito
if ($esCredito || $esDebito) {
    $html .= '
    <!-- Motivo de la nota -->
    <table width="100%" cellpadding="3" style="font-size: 9px;">
        <tr>
            <td class="bordered header-bg">
                <strong>MOTIVO:</strong> ' . ($cabecera[0]['tm_descrip'] ?? 'NO ESPECIFICADO') . '
            </td>
        </tr>
    </table>
    ...
    <table width="100%" cellpadding="3" style="font-size: 9px;">
        <tr>
            <td width="50%" class="bordered">
                <strong>DOCUMENTO ORIGINAL N°:</strong> ' . ($cabecera[0]['vc_nro_factura'] ?? 'SIN REFERENCIA') . '<br>
                <strong>FECHA DOC. ORIGINAL:</strong> ' . ($cabecera[0]['fecha_original'] ?? 'DD/MM/AAAA') . '
            </td>
            <td width="50%" class="bordered">
    ...
    <tr>
        ...
        <td width="30%">
            <table width="100%" cellpadding="3">
                ...
                <tr>
                    <td class="border-top text-right bold large" ' . $colorTotal . '>
                        <strong>' . ($esCredito ? 'TOTAL A CREDITAR:' : 'TOTAL A PAGAR:') . '</strong>
                    </td>
                    <td class="border-top text-right bold large" ' . $colorTotal . '>
                        ' . ($esCredito ? '(' . number_format($totalGeneral, 0, ",", ".") . ')' : number_format($totalGeneral, 0, ",", ".")) . '
                    </td>
                </tr>
            </table>
        </td>
...
<table width="100%" cellpadding="3" style="font-size: 9px;">
    <tr>
        <td class="bordered">
            <strong>TOTAL EN LETRAS:</strong> ' . convertirNumeroLetras($totalGeneral) . ($esCredito ? ' (A FAVOR DEL CLIENTE)' : '') . '
        </td>
    </tr>
</table>
...
// Nombre del archivo según tipo de nota
$nombreArchivo = 'nota_' . strtolower($tipoNota) . '_' . $cabecera[0]['not_nro_documento'] . '.pdf';
