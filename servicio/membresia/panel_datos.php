...
?>
    <div class="card card-primary">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                <label>N° Inscripcion</label>
                <input type="number" value="0" class="form-control" id="cpre_numero" disabled>
            </div>
            ...
            <div class="form-group">
                <label>Fecha</label>
                <input type="date" value="<?php echo date('Y-m-d'); ?>" class="form-control" id="cpre_fecha" disabled>
            </div>

            <div class="form-group">
                <label>Valido Hasta</label>
                <input type="date" value="" class="form-control" id="cpre_validez">
            </div>
...
<?php
    ...
    if ($id_mem == '-2') { //SE TRATA DEL ULTIMO PEDIDO
        $presupuestos = pg_fetch_all(pg_query($conn, "SELECT * FROM v_compras_presupuestos WHERE id_mem = (select max(id_mem) from compras_presupuestos_cabecera where  id_sucursal = $id_sucursal);"));
        ...
        $presupuestos = pg_fetch_all(pg_query($conn, "SELECT * FROM v_compras_presupuestos WHERE id_mem = $id_mem;"));
        $pedidos = pg_fetch_all(pg_query($conn, "SELECT * FROM v_pedidos_compra WHERE  estado = 'CONFIRMADO';"));
        $consolidacion = pg_fetch_all(pg_query($conn, "SELECT * FROM v_compras_presupuestos_consolidacion WHERE  id_mem = $id_mem;"));
    }
    $presupuestos_detalles = pg_fetch_all(pg_query($conn, "SELECT * FROM v_compras_presupuestos_detalles WHERE id_mem = $id_mem ORDER BY item_descrip, mar_descrip;"));
    $presupuestos_pedidos = pg_fetch_all(pg_query($conn, "SELECT * FROM v_compras_presupuestos_pedidos where id_mem = $id_mem ORDER BY  item_descrip, mar_descrip;"));
    $disabled = 'disabled';
...
?>
    <div class="card">
        <div class="card-body">
            ...
            <button class="btn btn-primary text-white" onclick="modalConsolidacion(<?php echo $presupuestos[0]['id_mem']; ?>);" id="btn-modal-secund-cerrar"><i class="fas fa-table-tree"></i> Consolidacion</button>
    ...
    <div class="row">
        <div class="card card-primary col-12">
            ...
            <div class="card-body">
                <input type="hidden" value="<?php echo $presupuestos[0]['id_mem']; ?>" id="id_mem">
            ...
            <div class="card-body">
                <?php if (!empty($presupuestos_detalles)) { ?>
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php $total = 0;
                            foreach ($presupuestos_detalles as $d) {
        ...
        <?php if ($presupuestos[0]['estado'] == 'PENDIENTE') {
            $articulos = pg_fetch_all(pg_query($conn, "SELECT * FROM v_items WHERE estado = 'ACTIVO' AND id_item NOT IN (select id_item from v_compras_presupuestos_detalles WHERE id_mem = " . $presupuestos[0]['id_mem'] . ") ORDER BY item_descrip;"))
        ?>
            ...
            <div class="card-body">
                ...
                <table class="table table-bordered">
                    ...
                    <?php if (!empty($presupuestos_pedidos)) { ?>
                        <tbody>
                            <?php $total = 0;
                            foreach ($presupuestos_pedidos as $d) {
                                $total = $total + ($d['precio'] * $d['cantidad']) ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($presupuestos[0]['estado'] == 'PENDIENTE') { ?>
                                            <button class="btn btn-warning text-white" onclick="modificar_detalle(<?php //echo $d['id_item']; 
