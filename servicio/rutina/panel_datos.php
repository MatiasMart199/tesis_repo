...
<?php
} else if ($id_rut == '0') { //CUANDO SE PRESIONA EL BOTON AGREGAR
...
?>
    <div class="card card-primary">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                <label>Fecha</label>
                <input type="date" value="<?php echo date('Y-m-d'); ?>" class="form-control" id="rut_fecha">
            </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="rut_observacion"></textarea>
            </div>
...
<?php
    ...
    if ($id_rut == '-2') { //SE TRATA DEL ULTIMO PEDIDO
        $cabecera = pg_fetch_all(pg_query($conn, "SELECT * FROM v_serv_rutinas_cab WHERE id_rut = (select max(id_rut) from serv_rutinas_cab where  id_sucursal = $id_sucursal);"));
        ...
        $cabecera = pg_fetch_all(pg_query($conn, "SELECT * FROM v_serv_rutinas_cab WHERE id_rut = $id_rut;"));
    }
    $detalles = pg_fetch_all(pg_query($conn, "SELECT * FROM v_serv_rutinas_det WHERE id_rut = $id_rut ORDER BY id_rut;"));
    //$footer = pg_fetch_all(pg_query($conn, "SELECT imc, grasa_corporal FROM v_serv_mediciones_foot WHERE id_rut = $id_rut ORDER BY id_rut;"));
    $disabled = 'disabled';
    ...
    <div class="row">
        <div class="card card-primary col-12">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    <label>Fecha</label>
                    <input type="date" value="<?= $cabecera[0]['rut_fecha']; ?>" class="form-control" id="rut_fecha">
                </div>
                ...
                <div class="form-group">
                    <label>Observación</label>
                    <textarea class="form-control" id="rut_observacion"><?= $cabecera[0]['rut_observacion']; ?></textarea>
                </div>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><?php echo $d['ejercicio']; ?></td>
                                    <td><?php echo $d['serie']; ?></td>
                                    <td><?php echo $d['repeticion']; ?></td>
        ...
        <?php if ($cabecera[0]['estado'] == 'PENDIENTE') {
            $actividades = pg_fetch_all(pg_query($conn, "SELECT * FROM actividades WHERE estado = 'ACTIVO' AND act_tipo = 'RUTINA' AND id_act NOT IN (select id_act from serv_rutinas_det WHERE id_rut = " . $cabecera[0]['id_rut'] . ") ORDER BY act_descrip;"))
        ?>
                ...
                <div class="card-body">
                    <?php if (!empty($actividades)) { ?>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" value="" class="form-control" id="agregar_ejercicio">
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>Serie</label>
                                    <input type="number" value="" class="form-control" id="agregar_serie">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>Repeticion</label>
                                    <input type="number" value="" class="form-control" id="agregar_repeticion">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>Peso</label>
                                    <input type="number" value="" class="form-control" id="agregar_peso">
                                </div>
                            </div>
                        </div>
