...
--
-- Name: sp_acceso(integer, character varying, character varying, character varying, timestamp without time zone, character varying); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_acceso(xid_usuario integer DEFAULT NULL::integer, xacc_login character varying DEFAULT NULL::character varying, xacc_contrasena character varying DEFAULT NULL::character varying, xacc_ip character varying DEFAULT NULL::character varying, xacc_fecha timestamp without time zone DEFAULT NULL::timestamp without time zone, usuario character varying DEFAULT NULL::character varying) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
  INSERT INTO accesos (
...
ALTER FUNCTION public.sp_acceso(xid_usuario integer, xacc_login character varying, xacc_contrasena character varying, xacc_ip character varying, xacc_fecha timestamp without time zone, usuario character varying) OWNER TO postgres;

--
-- Name: sp_actualizar_precio_compra(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_actualizar_precio_compra() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    -- Actualizar el precio_compra en la tabla items
    UPDATE items 
...
BEGIN 
    IF operacion = 1 THEN 
    	--INCERTAR CABECERA--
        INSERT INTO serv_alimentaciones_cab (id_ali,ali_fecha,ali_fecha_fin,ali_objetivo,ali_dias,ali_observacion,id_cliente,id_sucursal,id_funcionario,estado,auditoria)
        VALUES (
    ...
    ELSIF operacion = 5 THEN 
	  		...
	  		-- INSERTAR DETALLES
        INSERT INTO serv_alimentaciones_det (id_ali,id_act,id_res,cantidad,calorias,carbohidratos,proteinas,auditoria)
        VALUES (xid_ali, xid_act, xid_res, xcantidad, xcalorias,xcarbohidratos,xproteinas,
       'INSERCION DETALLE/' || usuario || '/' || NOW());
	    RAISE NOTICE 'COMIDA AÑADIDO CON ÉXITO';
    ...
    -- MODIFICAR DETALLE --
        ...
        RAISE NOTICE 'COMIDA MODIFICADO CON ÉXITO';
       
    ELSIF operacion = 7 THEN 
    -- ELIMINAR DETALLE
    DELETE FROM serv_alimentaciones_det
        WHERE id_ali = xid_ali AND id_act = xid_act;
        RAISE NOTICE 'COMIDA ELIMINADO CON ÉXITO';
...
--
-- Name: sp_aperturas_cierres(integer, timestamp without time zone, timestamp without time zone, numeric, numeric, numeric, numeric, numeric, numeric, integer, integer, integer, integer, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_aperturas_cierres(xid_vac integer, xvac_fecha_ape timestamp without time zone, xvac_fecha_cie timestamp without time zone, xvac_monto_efec numeric, xvac_monto_cheq numeric, xvac_monto_tarj numeric, xvac_monto_tranf numeric, xvac_monto_ape numeric, xvac_monto_cie numeric, xid_caja integer, xid_sucursal integer, xid_funcionario integer, xid_fun_solicitante integer, usuario character varying, operacion integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
declare 
d RECORD;
BEGIN
    IF operacion = 1 THEN 
        ...
        INSERT INTO vent_aperturas_cierres (id_vac,id_caja,id_sucursal,id_funcionario,vac_fecha_ape,vac_fecha_cie,vac_monto_efec,vac_monto_cheq,vac_monto_tarj,vac_monto_tranf,vac_monto_ape,vac_monto_cie,estado,auditoria)
        VALUES (
        ...
        RAISE NOTICE 'CAJA ABIERTA CON ÉXITO';
    ...
    ELSIF operacion = 2 THEN 
		...
		SET 
		    ...
		    vac_monto_tranf = xvac_monto_tranf,
        ...
        RAISE NOTICE 'CAJA CERRADA CON ÉXITO';
       
       
      insert into vent_recaudacion_depositar(id_vrd,id_vac,id_caja,id_sucursal,id_funcionario,vrd_monto_cie,vrd_fecha,estado,auditoria)
      values(
    ...
    ELSIF operacion = 4 THEN 
     ...
     insert into vent_arqueo_control (id_varq,arq_fecha,id_vac,id_caja,id_sucursal,id_funcionario,id_fun_solicitante,arq_monto_efec,arq_monto_cheq,arq_monto_tarj,arq_monto_tranf,estado,auditoria)values
    ...
    RAISE NOTICE 'ARQUEO AÑADIDO CON ÉXITO';
    
    elseif operacion = 6 then
    	select id_varq into d from v_arqueo_resumen where id_vac = xid_vac; 
    	delete from vent_arqueo_control vac where id_varq = d.id_varq;
    RAISE NOTICE 'ARQUEO ELIMINADO CON ÉXITO';
    ELSE
...
ALTER FUNCTION public.sp_aperturas_cierres(xid_vac integer, xvac_fecha_ape timestamp without time zone, xvac_fecha_cie timestamp without time zone, xvac_monto_efec numeric, xvac_monto_cheq numeric, xvac_monto_tarj numeric, xvac_monto_tranf numeric, xvac_monto_ape numeric, xvac_monto_cie numeric, xid_caja integer, xid_sucursal integer, xid_funcionario integer, xid_fun_solicitante integer, usuario character varying, operacion integer) OWNER TO postgres;
...
--
-- Name: sp_clientes(integer, character varying, character varying, character varying, character varying, character varying, character varying, date, character varying, integer, integer, integer, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_clientes(idcliente integer, pernombre character varying, perapellido character varying, perruc character varying, perci character varying, perdireccion character varying, percorreo character varying, perfenaci date, pertelefono character varying, idciudad integer, idecivil integer, idgenero integer, usuario character varying, operacion integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
declare 
v_id_persona INT;
BEGIN 
    IF operacion = 1 THEN -- INSERCION
           -- 1. Validar si ya es cliente registrado (usando la vista)
    PERFORM 1 FROM v_clientes WHERE per_ruc = perruc OR per_ci = perci;
    IF FOUND THEN
        RAISE EXCEPTION 'ESTE CLIENTE YA FUE REGISTRADO';
    END IF;
	
   	    IF btrim(coalesce(pernombre, '')) = '' or 
	    btrim(coalesce(perapellido::text, '')) = '' OR
       btrim(coalesce(perdireccion::text, '')) = '' OR
       btrim(coalesce(percorreo::text, '')) = ''OR
       btrim(coalesce(pertelefono::text, '')) = '' THEN
	      RAISE EXCEPTION 'EL ESTE CAMPO NO PUEDE ESTAR VACÍO';
	    END IF;
    -- 2. Buscar si la persona ya existe
    SELECT id_persona INTO v_id_persona FROM personas WHERE per_ruc = perruc OR per_ci = perci LIMIT 1;

    -- 3. Si no existe la persona, insertar
    IF NOT FOUND THEN
        INSERT INTO personas (
            id_persona, per_nombre, per_apellido, per_ruc, per_ci,
            per_direccion, per_correo, per_fenaci, per_telefono,
            persona_fisica, estado, auditoria, id_ciudad, id_ecivil, id_genero
        )
        VALUES (
    ...
    INSERT INTO clientes (
        id_cliente, id_persona, estado, auditoria
    )
    VALUES (
        (SELECT COALESCE(MAX(id_cliente), 0) + 1 FROM clientes),
        v_id_persona,
        'ACTIVO',
        'INSERCION/' || usuario || '/' || NOW()
    );
    ...
    ELSIF operacion = 2 THEN-- MODIFICACION
    ...
    ELSIF operacion = 3 THEN -- ACTIVAR
        UPDATE clientes
        SET estado = 'ACTIVO',
    ...
    ELSIF operacion = 4 THEN -- INACTIVAR
        UPDATE clientes
        SET estado = 'INACTIVO',
...
ALTER FUNCTION public.sp_clientes(idcliente integer, pernombre character varying, perapellido character varying, perruc character varying, perci character varying, perdireccion character varying, percorreo character varying, perfenaci date, pertelefono character varying, idciudad integer, idecivil integer, idgenero integer, usuario character varying, operacion integer) OWNER TO postgres;
...
BEGIN 
    ...
    ELSIF operacion = 5 THEN -- INSERTAR DETALLE
	    ...
	    PERFORM * FROM stock WHERE id_item = xid_item AND id_sucursal = xid_deposito;
	    if NOT FOUND THEN 
	        INSERT INTO stock (id_item, id_sucursal)
	        VALUES (xid_item,xid_deposito);
	    END IF;
        ...
        END IF;
        -- Inserta el detalle 
        INSERT INTO comp_ajustes_det (id_caju, id_item,id_sucursal, id_motivo, cantidad, precio, auditoria)
        VALUES (xid_caju, xid_item,xid_deposito, xid_motivo, xcantidad, (SELECT precio_compra FROM items WHERE id_item = xid_item), 
       'INSERCION/' || usuario || '/' || NOW());
...
--
-- Name: sp_comp_nota(integer, date, date, character varying, integer, integer, integer, integer, character varying, character varying, character varying, integer, integer, numeric, numeric, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_comp_nota(xid_not integer, xnot_fecha date, xnot_fecha_docu date, xnot_tipo_nota character varying, xid_cc integer, xid_sucursal integer, xid_funcionario integer, xid_proveedor integer, xnot_vehiculo character varying, xnot_chofer character varying, xnot_nro_documento character varying, xid_tm integer DEFAULT NULL::integer, xid_item integer DEFAULT NULL::integer, xcantidad numeric DEFAULT NULL::numeric, xmonto numeric DEFAULT NULL::numeric, usuario character varying DEFAULT NULL::character varying, operacion integer DEFAULT NULL::integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    d RECORD;
BEGIN 
    IF operacion = 1 THEN -- Insertar compra_pedido_cabecera
        ...
        -- Inserta 
        INSERT INTO comp_nota_cab (id_not,not_fecha,not_fecha_docu,not_tipo_nota,id_cc,id_sucursal,id_funcionario,id_proveedor,not_vehiculo,not_chofer,not_nro_documento,id_tm,estado,auditoria)
    ...
    ELSIF operacion = 2 THEN -- Modificar compra_pedido_cabecera
        ...
        not_vehiculo = xnot_vehiculo,
        not_chofer = xnot_chofer,
        not_nro_documento = xnot_nro_documento,
        auditoria = COALESCE(auditoria, '') || CHR(13) || 'MODIFICACION/' || usuario || '/' || NOW()
...
ALTER FUNCTION public.sp_comp_nota(xid_not integer, xnot_fecha date, xnot_fecha_docu date, xnot_tipo_nota character varying, xid_cc integer, xid_sucursal integer, xid_funcionario integer, xid_proveedor integer, xnot_vehiculo character varying, xnot_chofer character varying, xnot_nro_documento character varying, xid_tm integer, xid_item integer, xcantidad numeric, xmonto numeric, usuario character varying, operacion integer) OWNER TO postgres;
...
BEGIN 
    ...
    --------------------------------------------------------------------------------------------------------------------
    v_id_deposito_origen:= xid_deposito_ori;
...
-- Comprueba que existen detalles asociados a la transferencia
	...
	-- Recorremos cada detalle de la transferencia
	    FOR d IN 
	        SELECT id_item, id_sucursal AS id_deposito_destino, cantidad 
	        FROM comp_transfers_det 
	        WHERE id_tra = xid_tra
	    loop
		    
		IF (SELECT stock_cantidad FROM stock WHERE id_sucursal = v_id_deposito_origen AND id_item = d.id_item) <= d.cantidad THEN
    	RAISE EXCEPTION 'STOCK INSUFICIENTE'; 
		END IF;
	
	        -- 🟢 1. Verificar si existe el stock en el depósito destino
	        IF NOT EXISTS ( SELECT 1 FROM stock WHERE id_sucursal = d.id_deposito_destino AND id_item = d.id_item) THEN
	            -- No existe → Insertar
	            INSERT INTO stock (id_sucursal, id_item, stock_cantidad, estado)
	            VALUES (d.id_deposito_destino, d.id_item, d.cantidad, 'ACTIVO');
	
	            --RAISE NOTICE 'INSERTADO: Item %, % unidades en depósito destino %',
	                --d.id_item, d.cantidad, d.id_deposito_destino;
	        ELSE
	            -- Ya existe → Actualizar sumando cantidad
	            UPDATE stock
	            SET stock_cantidad = stock_cantidad + d.cantidad,
	                estado = 'ACTIVO'
	            WHERE id_sucursal = d.id_deposito_destino 
	              AND id_item = d.id_item;
	
	            --RAISE NOTICE 'ACTUALIZADO: Item %, +% unidades en depósito destino %',
	                --d.id_item, d.cantidad, d.id_deposito_destino;
	        END IF;
	
	        -- 🔴 2. Verificar existencia de stock en el depósito origen
	        IF EXISTS (
	            SELECT 1 FROM stock 
	            WHERE id_sucursal = v_id_deposito_origen 
	              AND id_item = d.id_item
	        ) THEN
	            -- Resta la cantidad
	            UPDATE stock
	            ...
	            --RAISE NOTICE 'DESCONTADO: Item %, -% unidades en depósito origen %',
	                --d.id_item, d.cantidad, v_id_deposito_origen;
	        ELSE
	            -- Si no existe en el origen → error
	            --RAISE EXCEPTION 'El item % no existe en el depósito origen (%)', 
	                --d.id_item, v_id_deposito_origen;
	        END IF;
	    END LOOP;				
	    
	       -- 🟣 3. Actualizar la cabecera de la transferencia
	    UPDATE comp_transfers_cab 
	    SET tra_fecha_recep = xtra_fecha_recep,
	        estado = 'CONFIRMADO',
	        auditoria = COALESCE(auditoria, '') || CHR(13) || 'CONFIRMACION/' || usuario || '/' || NOW()
	    WHERE id_tra = xid_tra;
    ...
    ELSIF operacion = 5 THEN -- Insertar 
        ...
        PERFORM * FROM stock WHERE id_item = xid_item AND id_sucursal = xid_deposito_des;
	    if NOT FOUND THEN 
	        INSERT INTO stock (id_item, id_sucursal,stock_cantidad,estado)
	        VALUES (xid_item,xid_deposito_des,0,'ACTIVO');
	    END IF;
        
        -- Inserta el detalle
        INSERT INTO comp_transfers_det (id_tra,id_item,id_sucursal,cantidad,auditoria)
        VALUES (xid_tra, xid_item,xid_deposito_des, xcantidad,'INSERCION/' || usuario || '/' || NOW());
...
--
-- Name: sp_compras(integer, date, date, integer, character varying, character varying, character varying, integer, numeric, numeric, numeric, numeric, numeric, integer, integer, integer, integer, integer, integer, integer, integer, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_compras(xid_cc integer, xcc_fecha date, xcc_fecha_emi date, xcc_intervalo integer, xcc_nro_factura character varying, xcc_timbrado character varying, xcc_tipo_factura character varying, xcc_cuota integer, xiva5 numeric, xiva10 numeric, xexenta numeric, xmonto numeric, xsaldo numeric, xid_sucursal integer, xid_deposito integer, xid_funcionario integer, xid_proveedor integer, xid_item integer, xcantidad integer, xprecio integer, xid_corden integer, usuario character varying, operacion integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    d RECORD;
...
PERFORM * FROM compras_cabecera WHERE id_sucursal = xid_sucursal AND estado = 'PENDIENTE';
    	...
    	--INCERTAR CABECERA--
        INSERT INTO compras_cabecera (id_cc,cc_fecha,cc_fecha_emi,cc_tipo_factura,cc_cuota,id_sucursal,id_funcionario,id_proveedor,estado,auditoria,cc_intervalo, cc_nro_factura, cc_timbrado)
        VALUES (
...
PERFORM * FROM compras_detalle WHERE id_cc = xid_cc AND id_item = xid_item;
   ...
   PERFORM * FROM stock WHERE id_item = xid_item AND id_sucursal = xid_deposito;
    ...
    END IF;
	  		...
	  		-- INSERTAR DETALLES
        INSERT INTO compras_detalle (id_cc, id_item, id_sucursal, cantidad, precio)
        VALUES (xid_cc, xid_item,xid_deposito, xcantidad, xprecio);
	    RAISE NOTICE 'PRODUCTO AÑADIDO CON ÉXITO';
...
ALTER FUNCTION public.sp_compras(xid_cc integer, xcc_fecha date, xcc_fecha_emi date, xcc_intervalo integer, xcc_nro_factura character varying, xcc_timbrado character varying, xcc_tipo_factura character varying, xcc_cuota integer, xiva5 numeric, xiva10 numeric, xexenta numeric, xmonto numeric, xsaldo numeric, xid_sucursal integer, xid_deposito integer, xid_funcionario integer, xid_proveedor integer, xid_item integer, xcantidad integer, xprecio integer, xid_corden integer, usuario character varying, operacion integer) OWNER TO postgres;
...
BEGIN 
    ...
    ELSIF operacion = 3 THEN -- Confirmar compra_pedido_cabecera
       ...
       IF NOT FOUND THEN
        ...
        -- Actualiza el estado del pedido a 'CONFIRMADO'.
        UPDATE compras_pedidos_cabecera SET estado = 'CONFIRMADO',cp_fecha_aprob = CURRENT_TIMESTAMP, auditoria = COALESCE(auditoria, '') || CHR(13) || 'CONFIRMACION/' || usuario || '/' || NOW() 
        WHERE id_cp = cid_cp;
...
--
-- Name: sp_items(integer, character varying, integer, integer, integer, integer, integer, character varying, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_items(xid_item integer, xitem_descrip character varying, xprecio_compra integer, xprecio_venta integer, xid_mar integer, xid_tip_item integer, xid_tip_impuesto integer, xitem_concepto character varying DEFAULT NULL::character varying, usuario character varying DEFAULT NULL::character varying, operacion integer DEFAULT NULL::integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN 
	IF operacion = 1 THEN --INSERCION 
		PERFORM * FROM items WHERE item_descrip = UPPER(TRIM(xitem_descrip));
		IF FOUND THEN 
			RAISE EXCEPTION 'ESTE DATO YA FUE REGISTRADO';
		END IF;
		
	    IF btrim(coalesce(xitem_descrip, '')) = '' or 
	    btrim(coalesce(xprecio_compra::text, '')) = '' OR
       btrim(coalesce(xprecio_venta::text, '')) = '' THEN
	      RAISE EXCEPTION 'EL ESTE CAMPO NO PUEDE ESTAR VACÍO';
	    END IF;
		
		INSERT INTO items (id_item, 
							item_descrip, 
							precio_compra,
							precio_venta,
							id_mar,
							...
							estado, 
							auditoria)
		VALUES ((SELECT COALESCE(MAX(id_item), 0) + 1 FROM items),
				UPPER(TRIM(xitem_descrip)),
				xprecio_compra,
				xprecio_venta,
				xid_mar,
				xid_tip_item,
	...
	IF operacion = 2 THEN --MODIFICACION 
		PERFORM * FROM items WHERE item_descrip = UPPER(TRIM(xitem_descrip)) AND id_item != xid_item;
		IF FOUND THEN
			RAISE EXCEPTION 'ESTE DATO YA FUE REGISTRADO';
		END IF;
		...
		UPDATE items
		SET item_descrip = UPPER(TRIM(xitem_descrip)),
			precio_compra = xprecio_compra,
			precio_venta = xprecio_venta,
			id_mar = xid_mar,
			id_tip_item = xid_tip_item,
			id_tip_impuesto = xid_tip_impuesto,
			item_concepto = xitem_concepto,
			auditoria = COALESCE(auditoria, '') || CHR(13) || 'MODIFICACION' || usuario || '/' || NOW()
		WHERE id_item = xid_item; 
		RAISE NOTICE 'DATOS MODIFICADOS CON EXITO';
	END IF;
...
ALTER FUNCTION public.sp_items(xid_item integer, xitem_descrip character varying, xprecio_compra integer, xprecio_venta integer, xid_mar integer, xid_tip_item integer, xid_tip_impuesto integer, xitem_concepto character varying, usuario character varying, operacion integer) OWNER TO postgres;
...
BEGIN 
    IF operacion = 1 THEN 
        ...
        FOR d IN SELECT * FROM v_servicios_inscripciones_detalle  WHERE id_inscrip = xid_inscrip LOOP
            ...
            VALUES ((select max(id_mem) from serv_membresias_cab), d.id_inscrip, d.id_plan_servi, d.dia, d.precio);
        END LOOP;
    ...
    -- CONFIRMAR CABECERA
       ...
       --GENERAR CUOTA MEMBRESIA 
       SELECT * INTO d FROM v_serv_membresias_cab WHERE id_mem = xid_mem;
       INSERT INTO public.cuotas_membresias
		(id_cm, nro_cuota, cm_monto, cm_fecha_venc, cm_dni, id_mem,id_cliente, estado, auditoria)
		VALUES((SELECT COALESCE(MAX(id_cm), 0) + 1 FROM cuotas_membresias), 1, d.monto_total, d.mem_fecha + interval '1 day', d.per_ci::INTEGER,d.id_mem,d.id_cliente, 
		'PENDIENTE', 
		'INSERCION/' || usuario || '/' || NOW());
...
BEGIN 
    ...
    ELSIF operacion = 5 THEN 
       ...
       'INSERCION DETALLE/' || usuario || '/' || NOW());
	    ...
	    RAISE NOTICE 'COTIZACIÓN AÑADIDO CON ÉXITO';
    ...
    -- MODIFICAR DETALLE --
        ...
        RAISE NOTICE 'COTIZACIÓN MODIFICADO CON ÉXITO';
       
    ELSIF operacion = 7 THEN 
    -- ELIMINAR DETALLE
    DELETE FROM serv_presupuestos_det
        WHERE id_pre = xid_pre AND id_act = xid_act;
        RAISE NOTICE 'COTIZACIÓN ELIMINADO CON ÉXITO';
...
BEGIN 
    IF operacion = 1 THEN 
    	--INCERTAR CABECERA--
        INSERT INTO serv_rutinas_cab (id_rut,rut_fecha,rut_edad,rut_observacion,id_cliente,id_personal,id_sucursal,id_funcionario,estado,auditoria)
        VALUES (
    ...
    ELSIF operacion = 5 THEN 
       ...
       'INSERCION DETALLE/' || usuario || '/' || NOW(),
      	xejercicio);
	    RAISE NOTICE 'EJERCICIO AÑADIDO CON ÉXITO';
    ...
    -- MODIFICAR DETALLE --
        ...
        RAISE NOTICE 'EJERCICIO MODIFICADO CON ÉXITO';
       
    ELSIF operacion = 7 THEN 
    -- ELIMINAR DETALLE
    DELETE FROM serv_rutinas_det
        WHERE id_rut = xid_rut AND id_act = xid_act;
        RAISE NOTICE 'EJERCICIO ELIMINADO CON ÉXITO';
...
--
-- Name: sp_servicios_inscripciones(integer, date, integer, numeric, numeric, boolean, integer, integer, integer, integer, integer, integer, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_servicios_inscripciones(cid_inscrip integer, cins_aprobacion date, cins_edad integer, cins_peso numeric, cins_altura numeric, cins_seguro_medico boolean, cid_sucursal integer, cid_funcionario integer, cid_cliente integer, cid_es integer, cid_plan_servi integer, cdia integer, usuario character varying, operacion integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN 
    IF operacion = 1 THEN -- Insertar compra_pedido_cabecera
        ...
        END IF;
       	...
       	--if cins_aprobacion <= CURRENT_DATE then
       		--raise exception 'LA FECHA DE VENCIMIENTO NO PUEDE SER MENOR A LA FECHA ACTUAL';
       	--end if;
       	
        
        -- Inserta la cabecera del pedido.
        INSERT INTO servicios_inscripciones_cabecera (id_inscrip,ins_fecha,ins_aprobacion,ins_edad,ins_peso,ins_altura,ins_seguro_medico,id_sucursal,id_funcionario,id_cliente,estado,auditoria)
		VALUES ((SELECT COALESCE(MAX(id_inscrip), 0) + 1 FROM servicios_inscripciones_cabecera), CURRENT_TIMESTAMP,cins_aprobacion,cins_edad,cins_peso,cins_altura,cins_seguro_medico,cid_sucursal,cid_funcionario,cid_cliente,'PENDIENTE', 'INSERCION/' || usuario || '/' || NOW());
    ...
    ELSIF operacion = 2 THEN -- Modificar compra_pedido_cabecera
       ...
       --if cins_aprobacion <= CURRENT_DATE then
        ...
        ins_edad = cins_edad,
        ins_peso = cins_peso,
        ins_altura = cins_altura,
        ins_seguro_medico = cins_seguro_medico,
        auditoria = COALESCE(auditoria, '') || CHR(13) || 'MODIFICACION/' || usuario || '/' || NOW()
    ...
    ELSIF operacion = 5 THEN -- Insertar compra_pedido_detalle
        ...
        IF FOUND THEN
            RAISE EXCEPTION 'ESTE PLAN YA HA SIDO REGISTRADO EN LA INSCRIPCIÓN';
        END IF;
        
        -- Inserta el detalle del pedido.
        INSERT INTO servicios_inscripciones_detalle (id_inscrip,id_plan_servi,dia, precio)
        VALUES (cid_inscrip, cid_plan_servi, cdia, (SELECT precio_con_promocion FROM v_planes_servicios WHERE id_plan_servi = cid_plan_servi));
    ...
    ELSIF operacion = 7 THEN -- Eliminar compra_pedido_detalle
        ...
        RAISE NOTICE 'PLAN ELIMINADO CON ÉXITO';
   		...
   		PERFORM * FROM clientes_estados_salud WHERE id_cliente = cid_cliente AND id_es = cid_es;
        IF FOUND THEN
            RAISE EXCEPTION 'ESTE REGISTRO YA A SIDO REGISTRADO EN LA INSCRIPCIÓN';
        END IF;
       
       insert into clientes_estados_salud (id_cliente, id_es, estado, auditoria) 
       values (cid_cliente, cid_es, 'ACTIVO',
      'INSERCION/' || usuario || '/' || NOW());
      raise notice 'ESTADO DE SALUD AGREGADO';
...
ALTER FUNCTION public.sp_servicios_inscripciones(cid_inscrip integer, cins_aprobacion date, cins_edad integer, cins_peso numeric, cins_altura numeric, cins_seguro_medico boolean, cid_sucursal integer, cid_funcionario integer, cid_cliente integer, cid_es integer, cid_plan_servi integer, cdia integer, usuario character varying, operacion integer) OWNER TO postgres;
...
--
-- Name: sp_validar_campos(text[]); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_validar_campos(VARIADIC campos text[]) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
    IF EXISTS (
        SELECT 1
        FROM unnest(campos) AS campo
        WHERE COALESCE(TRIM(campo), '') = ''
    ) THEN
        RAISE EXCEPTION 'EXISTEN CAMPOS VACÍOS';
    END IF;
END;
$$;


ALTER FUNCTION public.sp_validar_campos(VARIADIC campos text[]) OWNER TO postgres;

--
-- Name: sp_vent_cobros(integer, timestamp without time zone, integer, integer, integer, integer, integer, integer, integer, numeric, numeric, character varying, date, numeric, character varying, integer, integer, character varying, date, numeric, integer, character varying, numeric, character varying, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_vent_cobros(xid_cob integer, xcob_fecha timestamp without time zone, xid_vac integer, xid_caja integer, xid_vc integer, xid_sucursal integer, xid_funcionario integer, xid_cue integer, xid_fc integer, xcob_monto_efe numeric, xmonto numeric, xche_nro_cheque character varying, xche_vencimiento date, xche_monto numeric, xche_tipo_cheque character varying, xid_ee integer, xid_ee_des integer, xtar_nro_tarjeta character varying, xtar_vencimiento date, xtar_monto numeric, xid_mt integer, xtra_nro_cuenta character varying, xtra_monto numeric, xtra_motivo character varying, usuario character varying, operacion integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    d RECORD;
...
BEGIN 
    IF operacion = 1 THEN -- Insertar compra_pedido_cabecera
        -- Comprueba si hay pedidos pendientes de confirmación para esta sucursal.
        PERFORM * FROM vent_cobros_cab WHERE id_sucursal = xid_sucursal AND estado = 'PENDIENTE'; 
        IF FOUND THEN
            RAISE EXCEPTION 'EXISTEN MOVIMIENTO PENDIENTES DE CONFIRMACIÓN';
        END IF;
        
        -- Inserta 
        INSERT INTO vent_cobros_cab (id_cob,id_vac,id_caja,id_sucursal,id_funcionario,cob_fecha,estado,auditoria,id_vc)
		VALUES ((SELECT COALESCE(MAX(id_cob), 0) + 1 FROM vent_cobros_cab),  
				xid_vac,
				xid_caja,
				xid_sucursal, 
				xid_funcionario,
    ...
    ELSIF operacion = 2 THEN -- Modificar compra_pedido_cabecera
        -- Comprueba si el pedido existe y está pendiente.
        PERFORM * FROM vent_cobros_cab WHERE id_cob = xid_cob AND estado = 'PENDIENTE'; 
        IF NOT FOUND THEN
            RAISE EXCEPTION 'ESTE MOVIMIENTO NO SE PUEDE MODIFICAR O NO EXISTE';
        END IF;
        
        -- Actualiza la cabecera del pedido.
        UPDATE vent_cobros_cab SET 
        id_vac = xid_vac, 
        id_caja = xid_caja,
        id_sucursal = xid_sucursal,
        id_funcionario = xid_funcionario,
        cob_fecha = xcob_fecha,
        auditoria = COALESCE(auditoria, '') || CHR(13) || 'MODIFICACION/' || usuario || '/' || NOW()
        WHERE id_cob = xid_cob;
    ...
    ELSIF operacion = 3 THEN -- Confirmar compra_pedido_cabecera
        ...
        IF NOT FOUND THEN
            RAISE EXCEPTION 'ESTE MOVIMIENTO NO SE PUEDE CONFIRMAR O NO EXISTE';
        END IF;
       PERFORM * FROM vent_cobros_det WHERE id_cob = xid_cob; 
       IF NOT FOUND THEN
            RAISE EXCEPTION 'ESTE MOVIMIENTO NO POSEE DETALLES CARGADOS';
        END IF;
       
       PERFORM 1 FROM v_vent_cobros_det WHERE id_cob = xid_cob AND cue_saldo > 0.001; 
	   IF FOUND THEN
	       RAISE EXCEPTION 'EXISTEN CUOTA QUE NO SE PAGO EN SU TOTALIDAD';
	   END IF;
      
       select id_cob,id_vac,total_efectivo,total_cheque,total_tarjeta,total_transfe,total_general into d from v_vent_cobros_montos where id_cob = xid_cob;
       update vent_aperturas_cierres 
       set vac_monto_efec = vac_monto_efec + d.total_efectivo,
	       vac_monto_cheq = vac_monto_cheq + d.total_cheque,
	       vac_monto_tarj = vac_monto_tarj + d.total_tarjeta,
	       vac_monto_tranf = vac_monto_tranf + d.total_transfe,
	       vac_monto_cie = vac_monto_cie + d.total_general
       where id_vac = d.id_vac;
       
        -- Actualiza el estado del pedido a 'CONFIRMADO'.
        UPDATE vent_cobros_cab SET estado = 'CONFIRMADO', auditoria = COALESCE(auditoria, '') || CHR(13) || 'CONFIRMACION/' || usuario || '/' || NOW() 
        WHERE id_cob = xid_cob;
        
        RAISE NOTICE 'DATOS CONFIRMADOS CON ÉXITO';
       
       --SELECT * from vent_cuentas_cobrar
       --ACTUALIZA SALDO DE CUOTA
       
       --UPDATE vent_cuentas_cobrar c
		--SET cue_saldo = c.cue_saldo - det.cob_monto_efe,
		    --estado = CASE 
		               --WHEN c.cue_saldo - det.cob_monto_efe <= 0 THEN 'PAGADO'::estados
		               --ELSE 'PENDIENTE'::estados
		             --END
		--FROM vent_cobros_det det
		--WHERE c.id_cue = det.id_cue
		 -- AND det.id_cob = xid_cob;
		 
		 
		FOR d IN SELECT * FROM v_vent_cobros_det WHERE id_cob = xid_cob  and cue_saldo <= 0.001 loop
		    UPDATE vent_cuentas_cobrar set estado = 'PAGADO'
        	WHERE id_cue = d.id_cue;
		END LOOP;
    ...
    ELSIF operacion = 4 THEN -- Anular compra_pedido_cabecera
        ...
        END IF;
       	 ...
       	 FOR d IN SELECT * FROM v_vent_cobros_det WHERE id_cob = xid_cob loop
		    UPDATE vent_cuentas_cobrar set cue_saldo = d.cue_monto
        	WHERE id_cue = d.id_cue;
		END LOOP;
       
        -- Actualiza el estado del pedido a 'ANULADO'.
        UPDATE vent_cobros_cab SET estado = 'ANULADO', auditoria = COALESCE(auditoria, '') || CHR(13) || 'ANULACION/' || usuario || '/' || NOW() 
        WHERE id_cob = xid_cob;
    ...
    ELSIF operacion = 5 THEN 
        ...
        PERFORM * FROM vent_cobros_det WHERE id_cob = xid_cob AND id_cue = xid_cue;
        IF FOUND THEN
            RAISE EXCEPTION 'ESTA CUENTA YA HA SIDO REGISTRADO';
        END IF;
       PERFORM * FROM vent_cuentas_cobrar WHERE id_cue = xid_cue AND cue_monto < xcob_monto_efe;  -- Cambiado a <
		IF FOUND THEN  -- Cambiado a FOUND
		    RAISE EXCEPTION 'EXCEDE EL MONTO DE LA CUOTA. Cuota: %, Intento cobrar: %', 
		        (SELECT cue_monto FROM vent_cuentas_cobrar WHERE id_cue = xid_cue),
		        xcob_monto_efe;
		END IF;
       IF xcob_monto_efe < 0 THEN
		    RAISE EXCEPTION 'EL MONTO DEL COBRO NO PUEDE SER NEGATIVO';
		END IF;
	
		PERFORM 1 FROM vent_cuentas_cobrar WHERE id_cue = xid_cue AND estado = 'PAGADO'; 
        IF FOUND THEN
            RAISE EXCEPTION 'ESTA CUENTA YA ESTA PAGADA';
        END IF;
	
		monto_cuota := xmonto - xcob_monto_efe;
		--perform * from vent_cuentas_cobrar where id_cue = xid_cue and cue_saldo > monto_cuota;
	      --if not found then
	      	--raise exception 'EL PAGO EXCEDE EL MONTO DE LA CUOTA';
	     --end if;
	    
        -- Inserta el detalle del pedido.
        INSERT INTO vent_cobros_det (id_cob,id_cue,monto,cob_monto_efe,auditoria)
        VALUES (xid_cob, xid_cue,xmonto, monto_cuota ,'INSERCION/' || usuario || '/' || NOW());
        
        RAISE NOTICE 'CUENTA AÑADIDO CON ÉXITO';
    ELSIF operacion = 6 THEN -- Modificar compra_pedido_detalle
        -- Comprueba si el pedido existe y está pendiente.
        PERFORM * FROM vent_cobros_cab WHERE id_cob = xid_cob AND estado = 'PENDIENTE';
        IF NOT FOUND THEN 
		...
		--perform * from vent_cuentas_cobrar where id_cue = xid_cue and cue_saldo > monto_cuota;
	     ...
	     --end if;
       IF xcob_monto_efe <= 0 THEN
		    RAISE EXCEPTION 'EL MONTO DEL COBRO DEBE SER MAYOR QUE CERO';
		END IF;

	    
        -- Actualiza la cantidad y el precio del detalle del pedido.
        UPDATE vent_cobros_det set id_fc = xid_fc, cob_monto_efe = xcob_monto_efe
        WHERE id_cob = xid_cob AND id_cue = xid_cue;
       
       if xid_fc = 4 then
	       update vent_cuentas_cobrar set cue_saldo = cue_saldo - xcob_monto_efe 
	       where id_cue = xid_cue;
       end if;
      
        RAISE NOTICE 'COBRO MODIFICADA CON ÉXITO';
    ELSIF operacion = 7 THEN -- Eliminar compra_pedido_detalle
        -- Comprueba si el pedido existe y está pendiente.
        PERFORM * FROM vent_cobros_cab WHERE id_cob = xid_cob AND estado = 'PENDIENTE';
        IF NOT FOUND THEN 
        ...
        -- Elimina el detalle del pedido.
        DELETE FROM vent_cobros_det WHERE id_cob = xid_cob AND id_cue = xid_cue;
        
        RAISE NOTICE 'CUENTA ELIMINADO CON ÉXITO';
       
       --------------------------------METODOS DE COBROS-------------------------------------------------  
       ---Cobro cheque
       elseif operacion = 8 then
       perform * from vent_cobros_det where id_cob = xid_cob and id_fc = 1;
	      if not found then
	      	raise exception 'NO ENCUENTRA EL METODO DE PAGO';
	     end if;
	    
	    --monto_cuota := xmonto - xche_monto;
     	--perform 1 from v_vent_cobros_det where id_cob = xid_cob and id_cue = xid_cue and cue_saldo >= xche_monto;
	      --if not found then
	      	--raise exception 'EL PAGO EXCEDE EL MONTO DE LA CUOTA';
	     --end if;
	    
	   IF xche_monto <= 0 THEN
		    RAISE EXCEPTION 'EL MONTO DEL COBRO DEBE SER MAYOR QUE CERO';
		END IF; 
	    
     insert into cobros_cheques (id_che,che_nro_cheque,che_vencimiento,che_monto,che_tipo_cheque,id_cob,id_ee,estado,auditoria)
     values((SELECT COALESCE(MAX(id_che), 0) + 1 FROM cobros_cheques),
    	xche_nro_cheque,xche_vencimiento,xche_monto,xche_tipo_cheque,xid_cob,xid_ee,'ACTIVO','INSERCION/' || usuario || '/' || NOW());
            RAISE NOTICE 'DATOS AÑADIDO CON ÉXITO';

       if xid_fc = 1 then
	       update vent_cuentas_cobrar set cue_saldo = cue_saldo - xche_monto 
	       where id_cue = xid_cue;
       end if;
      
    elseif operacion = 9 then
    	--perform * from vent_cobros_det where id_cob = xid_cob and id_fc = 2;
	      --if not found then
	      	--raise exception 'NO ENCUENTRA EL METODO DE PAGO';
	     --end if;
         --perform * from v_vent_cobros_det where id_cob = xid_cob and cue_saldo > xtar_monto;
	      --if not found then
	      	--raise exception 'EL PAGO EXCEDE EL MONTO DE LA CUOTA';
	     --end if;
    IF xtar_monto <= 0 THEN
		    RAISE EXCEPTION 'EL MONTO DEL COBRO DEBE SER MAYOR QUE CERO';
		END IF; 
     
     insert into cobros_tarjetas (id_ctar,tar_nro_tarjeta,tar_vencimiento,tar_monto,id_cob,id_ee,id_mt,estado,auditoria)
     values((SELECT COALESCE(MAX(id_ctar), 0) + 1 FROM cobros_tarjetas),
    xtar_nro_tarjeta,xtar_vencimiento,xtar_monto,xid_cob,xid_ee,xid_mt,'ACTIVO','INSERCION/' || usuario || '/' || NOW());
    		RAISE NOTICE 'DATOS AÑADIDO CON ÉXITO';
    	
       if xid_fc = 2 then
	       update vent_cuentas_cobrar set cue_saldo = cue_saldo - xtar_monto 
	       where id_cue = xid_cue;
       end if;
      
    elseif operacion = 10 then
    	perform * from vent_cobros_det where id_cob = xid_cob and id_fc = 3;
	      if not found then
	      	raise exception 'NO ENCUENTRA EL METODO DE PAGO';
	     end if;
	    --perform * from v_vent_cobros_det where id_cob = xid_cob and cue_saldo > xtra_monto;
	      --if not found then
	      	--raise exception 'EL PAGO EXCEDE EL MONTO DE LA CUOTA';
	     --end if;
	    
	    IF xtra_monto <= 0 THEN
		    RAISE EXCEPTION 'EL MONTO DEL COBRO DEBE SER MAYOR QUE CERO';
		END IF; 
     
     insert into cobros_transferencia (id_ctra,tra_nro_cuenta,tra_fecha,tra_monto,tra_motivo,id_cob,id_ee_ori,id_ee_des,estado,auditoria)
     values((SELECT COALESCE(MAX(id_ctra), 0) + 1 FROM cobros_transferencia),xtra_nro_cuenta,CURRENT_DATE,xtra_monto,xtra_motivo,xid_cob,xid_ee,xid_ee_des,'ACTIVO','INSERCION/' || usuario || '/' || NOW());
    		RAISE NOTICE 'DATOS AÑADIDO CON ÉXITO';
    	
    	if xid_fc = 3 then
	       update vent_cuentas_cobrar set cue_saldo = cue_saldo - xtra_monto 
	       where id_cue = xid_cue;
       end if;
    END IF;
END;
...
ALTER FUNCTION public.sp_vent_cobros(xid_cob integer, xcob_fecha timestamp without time zone, xid_vac integer, xid_caja integer, xid_vc integer, xid_sucursal integer, xid_funcionario integer, xid_cue integer, xid_fc integer, xcob_monto_efe numeric, xmonto numeric, xche_nro_cheque character varying, xche_vencimiento date, xche_monto numeric, xche_tipo_cheque character varying, xid_ee integer, xid_ee_des integer, xtar_nro_tarjeta character varying, xtar_vencimiento date, xtar_monto numeric, xid_mt integer, xtra_nro_cuenta character varying, xtra_monto numeric, xtra_motivo character varying, usuario character varying, operacion integer) OWNER TO postgres;

--
-- Name: sp_vent_nota(integer, timestamp without time zone, date, character varying, text, character varying, integer, integer, integer, integer, integer, integer, integer, numeric, numeric, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_vent_nota(xid_not integer, xnot_fecha_elab timestamp without time zone, xnot_fecha_emis date, xnot_tipo_nota character varying, xnot_observacion text, xnot_nro_documento character varying, xid_vc integer, xid_tm integer DEFAULT NULL::integer, xid_tim integer DEFAULT NULL::integer, xid_sucursal integer DEFAULT NULL::integer, xid_funcionario integer DEFAULT NULL::integer, xid_cliente integer DEFAULT NULL::integer, xid_item integer DEFAULT NULL::integer, xcantidad numeric DEFAULT NULL::numeric, xmonto numeric DEFAULT NULL::numeric, usuario character varying DEFAULT NULL::character varying, operacion integer DEFAULT NULL::integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    d RECORD;
...
BEGIN 
    IF operacion = 1 THEN 
        ...
        perform * FROM vent_nota_cab WHERE id_sucursal = 1 AND estado = 'PENDIENTE' AND not_tipo_nota ='CREDITO' OR not_tipo_nota ='DEBITO'; 
        IF FOUND THEN
          RAISE EXCEPTION 'EXISTEN MOVIMIENTO PENDIENTES DE CONFIRMACIÓN';
        END IF;
       select id_tm into c from v_ventas_cab where id_vc = xid_vc;
        IF c.id_tm = 3 THEN
            RAISE EXCEPTION 'NO SE PERMITE EMITIR NOTA DE CRÉDITO/DÉBITO PARA FACTURAS DE SERVICIO';
        END IF;
        
        -- Inserta 
        INSERT INTO vent_nota_cab (id_not,not_fecha_elab,not_fecha_emis,not_tipo_nota, id_tm,id_tim, not_observacion,not_nro_documento,id_vc,id_sucursal,id_funcionario,id_cliente ,estado,auditoria)
		VALUES ((SELECT COALESCE(MAX(id_not), 0) + 1 FROM vent_nota_cab), 
				now(), 
				xnot_fecha_emis,
				xnot_tipo_nota,
				xid_tm,
    ...
    ELSIF operacion = 3 THEN
       ...
       -- ACTUALIZAR STOCK CON UN FOR
      	--select * from items i 
    FOR d IN SELECT id_item, cantidad,id_sucursal, id_tip_item FROM v_vent_nota_det WHERE id_not = xid_not and id_tip_item <> 7 loop 
	    PERFORM * from stock  where id_item = d.id_item and stock_cantidad > 0;
	       if not found then 
	       		raise EXCEPTION 'STOCK INSUFICIENTE';
	       end if;
	    ...
	    ELSE
	        -- Insertar un nuevo registro en stock
	        INSERT INTO stock (id_sucursal, id_item, stock_cantidad, estado)
	        VALUES (d.id_sucursal, d.id_item, d.cantidad, 'ACTIVO');
	    END IF;
     ...
     END LOOP;
      ...
      --INSERTAR CUENTA A PAGAR
       --select * from v_vent_nota_cab
       select id_not,id_vc,monto_detalles,vc_cuota into c from v_vent_nota_cab where id_vc = xid_vc;
        FOR i IN 1..GREATEST(c.vc_cuota, 1) loop
	        monto_cuota := round(c.monto_detalles / c.vc_cuota, 0);
	        update vent_cuentas_cobrar set  
	        	cue_monto = monto_cuota, 
	        	cue_saldo = monto_cuota,
	            auditoria = COALESCE(auditoria, '') || CHR(13) || 'AJUSTADO/' || usuario || '/' || NOW()
	       where id_vc = c.id_vc;
    	END LOOP;
		
		    --Actualizar estado a 'PAGADO' si el saldo <= 0
	
	    FOR d in SELECT id_cue FROM vent_cuentas_cobrar WHERE id_vc = c.id_vc AND cue_saldo <= 0 LOOP
	        UPDATE vent_cuentas_cobrar SET estado = 'PAGADO'
	        WHERE id_cue = d.id_cue;
	    END LOOP;
    ...
    ELSIF operacion = 6 THEN
       ...
       -- Verifico si existe un item con cantidad o precio < 0
		--PERFORM 1 FROM vent_nota_det WHERE id_not = xid_not AND id_item = xid_item AND (cantidad < 0 OR precio < 0);
        IF xcantidad < 0 OR xmonto < 0 THEN
        	RAISE EXCEPTION 'NO SE PUEDE AJUSTAR VALORES NEGATIVOS';
        end if;
        
		UPDATE vent_nota_det 
		SET cantidad = xcantidad, 
		    precio   = xmonto
		WHERE id_not = xid_not 
		  AND id_item = xid_item;
...
ALTER FUNCTION public.sp_vent_nota(xid_not integer, xnot_fecha_elab timestamp without time zone, xnot_fecha_emis date, xnot_tipo_nota character varying, xnot_observacion text, xnot_nro_documento character varying, xid_vc integer, xid_tm integer, xid_tim integer, xid_sucursal integer, xid_funcionario integer, xid_cliente integer, xid_item integer, xcantidad numeric, xmonto numeric, usuario character varying, operacion integer) OWNER TO postgres;

--
-- Name: sp_vent_nota_remision(integer, timestamp without time zone, date, character varying, text, character varying, integer, integer, integer, integer, integer, integer, integer, integer, numeric, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_vent_nota_remision(xid_not integer, xnot_fecha_elab timestamp without time zone, xnot_fecha_emis date, xnot_tipo_nota character varying, xnot_observacion text, xnot_nro_documento character varying, xid_vc integer, xid_chofer integer DEFAULT NULL::integer, xid_vehiculo integer DEFAULT NULL::integer, xid_tim integer DEFAULT NULL::integer, xid_sucursal integer DEFAULT NULL::integer, xid_funcionario integer DEFAULT NULL::integer, xid_cliente integer DEFAULT NULL::integer, xid_item integer DEFAULT NULL::integer, xcantidad numeric DEFAULT NULL::numeric, usuario character varying DEFAULT NULL::character varying, operacion integer DEFAULT NULL::integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    d RECORD;
...
BEGIN 
    IF operacion = 1 THEN 
        ...
        PERFORM * FROM vent_nota_cab WHERE id_sucursal = xid_sucursal AND not_tipo_nota ='REMISION' AND estado = 'PENDIENTE'; 
        IF FOUND THEN
          RAISE EXCEPTION 'EXISTEN MOVIMIENTO PENDIENTES DE CONFIRMACIÓN';
        END IF;
       ...
       select id_tm into c from v_ventas_cab where id_vc = xid_vc;
        IF c.id_tm = 3 THEN
            RAISE EXCEPTION 'NO SE PERMITE EMITIR NOTA DE REMISIÓN PARA FACTURAS DE SERVICIO';
        END IF;
        
        -- Inserta 
        INSERT INTO vent_nota_cab (id_not,not_fecha_elab,not_fecha_emis,not_tipo_nota,id_tim, not_observacion,not_nro_documento,id_vc,id_vehiculo,id_chofer,id_sucursal,id_funcionario,id_cliente ,estado,auditoria)
		VALUES ((SELECT COALESCE(MAX(id_not), 0) + 1 FROM vent_nota_cab), 
				now(), 
				xnot_fecha_emis,
				xnot_tipo_nota,
    ...
    ELSIF operacion = 3 THEN
     ...
     -- AUMENTAR NUMERO DE INICIO DE TIMBRADO---------------------------------------------------
		UPDATE timbrados
		SET tim_num_inicio = tim_num_inicio + 1
		WHERE id_tim = xid_tim;
    ...
    ELSIF operacion = 5 THEN
        ...
        INSERT INTO vent_nota_det (id_not,id_item,cantidad,precio)
        VALUES (xid_not, xid_item, xcantidad, (SELECT precio_venta FROM items WHERE id_item = xid_item));
...
ALTER FUNCTION public.sp_vent_nota_remision(xid_not integer, xnot_fecha_elab timestamp without time zone, xnot_fecha_emis date, xnot_tipo_nota character varying, xnot_observacion text, xnot_nro_documento character varying, xid_vc integer, xid_chofer integer, xid_vehiculo integer, xid_tim integer, xid_sucursal integer, xid_funcionario integer, xid_cliente integer, xid_item integer, xcantidad numeric, usuario character varying, operacion integer) OWNER TO postgres;

--
-- Name: sp_ventas(integer, date, integer, character varying, character varying, integer, numeric, numeric, numeric, numeric, numeric, integer, integer, integer, integer, integer, integer, integer, integer, integer, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_ventas(xid_vc integer, xvc_fecha date, xvc_intervalo integer, xvc_nro_factura character varying, xvc_tipo_factura character varying, xvc_cuota integer, xiva5 numeric, xiva10 numeric, xexenta numeric, xmonto numeric, xsaldo numeric, xid_sucursal integer, xid_tim integer, xid_tm integer, xid_funcionario integer, xid_cliente integer, xid_item integer, xcantidad integer, xprecio integer, xid_vped integer, usuario character varying, operacion integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    d RECORD;
...
BEGIN 
    ...
    -------VALIDA SI EXISTE MOVIMIENTO PEDIENTE
    	...
    	END IF;
    	if xid_tm = 3 then
	    	PERFORM * FROM cuotas_membresias WHERE id_cliente = xid_cliente AND estado = 'PENDIENTE';
		    if NOT FOUND THEN
		    	RAISE EXCEPTION 'EL CLIENTE NO POSEE CUOTAS PENDIENTES';
	    	END IF;
		    if xvc_tipo_factura = 'CREDITO' THEN
		    	RAISE EXCEPTION 'LA FACTURA DEL SERVICIO DEBE SER AL CONTADO';
	    	END IF;
	    end if;
    
    	--INCERTAR CABECERA--
        INSERT INTO ventas_cab (id_vc,vc_fecha,vc_tipo_factura,vc_cuota,vc_intervalo,vc_nro_factura,id_tim,id_tm,id_sucursal,id_funcionario,id_cliente,estado,auditoria)
        VALUES (
        ...
        if xid_tm = 3 then
			select * into d from cuotas_membresias where id_cliente = xid_cliente and estado = 'PENDIENTE';
       		insert into ventas_servicio_det (id_vc,id_cm,nro_cuota,precio)
       		values((select max(id_vc) from ventas_cab), d.id_cm,d.nro_cuota,d.cm_monto);
	    end if;
...
PERFORM * FROM ventas_cab WHERE id_vc = xid_vc AND estado = 'PENDIENTE';
    ...
    ELSIF operacion = 3 THEN 
     PERFORM * FROM ventas_cab WHERE id_vc = xid_vc AND estado = 'PENDIENTE'; 
       ...
       IF NOT EXISTS (
		    SELECT 1 
		    FROM ventas_servicio_det 
		    WHERE id_vc = xid_vc
		    UNION
		    SELECT 1
		    FROM v_ventas_consolidacion
		    WHERE id_vc = xid_vc
		) THEN
		    RAISE EXCEPTION 'ESTE MOVIMIENTO NO POSEE DETALLES CARGADOS';
		END IF;
    ...
    FOR d IN SELECT id_item, catidad,id_sucursal FROM v_ventas_consolidacion WHERE id_vc = xid_vc loop 
      ...
      --INSERTAR CUENTA A PAGAR
        ...
        FOR i IN 1..GREATEST(xvc_cuota, 1) loop
	        monto_cuota := round(xmonto / xvc_cuota, 0);
	       
	        INSERT INTO vent_cuentas_cobrar (id_cue, id_vc,nro_cuota,fecha_intervalo, cue_monto, cue_saldo, estado, auditoria)
	        VALUES (
	            (SELECT COALESCE(MAX(id_cue), 0) + 1 FROM vent_cuentas_cobrar),
	            xid_vc,i,
	            xvc_fecha + (i * (xvc_intervalo || ' days')::interval),
	            monto_cuota, monto_cuota, 'PENDIENTE', 
	            'INSERCION/' || usuario || '/' || NOW()
	        );
    	END LOOP;
   ----------------------------------------------------------------------------------------------------------------------------------
	...
	--ACTUALIZA ESTADO DE CUOTA MEMBRESIA---------------------------------------------------
		if xid_tm = 3 then
		select * into d from ventas_cab where id_vc = xid_vc;
			UPDATE cuotas_membresias 
			SET estado = 'PAGADO'
...
CREATE TABLE public.accesos (
    ...
    auditoria text,
    acc_login character varying(70),
    acc_contrasena character varying,
    acc_ip character varying,
    acc_fecha timestamp without time zone
);
...
--
-- Name: clientes_estados_salud; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.clientes_estados_salud (
    id_cliente integer NOT NULL,
...
ALTER TABLE public.clientes_estados_salud OWNER TO postgres;

--
-- Name: cobros_cheques; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.cobros_cheques (
    id_che integer NOT NULL,
    che_nro_cheque character varying(50),
    che_vencimiento date,
    che_monto numeric,
    che_tipo_cheque character varying(70),
    id_cob integer,
    id_ee integer,
    estado public.estados,
    auditoria text
...
ALTER TABLE public.cobros_cheques OWNER TO postgres;

--
-- Name: cobros_tarjetas; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.cobros_tarjetas (
    id_ctar integer NOT NULL,
    tar_nro_tarjeta character varying(50),
...
ALTER TABLE public.cobros_tarjetas OWNER TO postgres;

--
-- Name: cobros_transferencia; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.cobros_transferencia (
    id_ctra integer NOT NULL,
    tra_nro_cuenta character varying(50),
    tra_fecha date,
    tra_monto numeric,
    tra_motivo character varying(100),
    id_cob integer,
    id_ee_ori integer,
    id_ee_des integer,
    estado public.estados,
...
ALTER TABLE public.cobros_transferencia OWNER TO postgres;
...
CREATE TABLE public.comp_nota_cab (
    ...
    auditoria text,
    id_tm integer,
    not_vehiculo character varying,
    not_chofer character varying,
    not_nro_documento character varying
);
...
--
-- Name: cuotas_membresias; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.cuotas_membresias (
    id_cm integer NOT NULL,
    nro_cuota integer,
    cm_monto numeric,
    cm_fecha_venc date,
    cm_dni character varying(20),
    id_mem integer,
    estado public.estados,
    auditoria text,
    id_cliente integer
...
ALTER TABLE public.cuotas_membresias OWNER TO postgres;
...
--
-- Name: entidades_adheridas; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.entidades_adheridas (
    id_ee integer NOT NULL,
    id_mt integer NOT NULL
);


ALTER TABLE public.entidades_adheridas OWNER TO postgres;
...
CREATE TABLE public.entidades_emisoras (
    id_ee integer NOT NULL,
    ee_razon_social character varying(100),
    ee_direccion character varying(150),
    ee_telefono character varying(30),
    ee_ruc character varying(20),
    ee_email character varying(70),
...
--
-- Name: estados_salud; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.estados_salud (
    id_es integer NOT NULL,
    es_nombre character varying(100),
    es_descrip text,
    estado public.estados,
    auditoria text
);
...
ALTER TABLE public.estados_salud OWNER TO postgres;

--
-- Name: formas_cobros; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.formas_cobros (
    id_fc integer NOT NULL,
    fc_descrip character varying(150),
    estado public.estados,
    auditoria text
);


ALTER TABLE public.formas_cobros OWNER TO postgres;
...
CREATE TABLE public.items (
    ...
    estado public.estados,
    auditoria text,
    item_concepto character varying(20)
);
...
--
-- Name: marcas_tarjetas; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.marcas_tarjetas (
    id_mt integer NOT NULL,
    mt_descrip character varying(150),
    estado public.estados,
    auditoria text
);


ALTER TABLE public.marcas_tarjetas OWNER TO postgres;
...
CREATE TABLE public.recetas (
    ...
    res_descrip character varying(50),
    res_ingrediente text,
    res_calorias numeric,
    res_carbohidratos numeric,
    res_proteinas numeric,
    estado public.estados,
    auditoria text
);
...
--
-- Name: tipos_duracion; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.tipos_duracion (
    id_td integer NOT NULL,
    td_descrip character varying(50),
    td_duracion numeric,
    estado public.estados,
    auditoria text
);
...
ALTER TABLE public.tipos_duracion OWNER TO postgres;
...
--
-- Name: tipos_movimientos; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.tipos_movimientos (
    id_tm integer NOT NULL,
    tm_descrip character varying(50),
    tm_codigo character varying(15),
    tm_afecta_inventario boolean,
    tm_afecta_contabilidad boolean,
    estado public.estados,
    auditoria text
);


ALTER TABLE public.tipos_movimientos OWNER TO postgres;
...
CREATE TABLE public.vent_arqueo_control (
    ...
    auditoria text,
    arq_monto_efec numeric,
    arq_monto_cheq numeric,
    arq_monto_tarj numeric,
    arq_monto_tranf numeric
);
...
--
-- Name: v_arqueo_resumen; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_arqueo_resumen AS
 SELECT ac.id_vac,
    arq.id_varq,
    ac.id_caja,
    c.caj_descrip,
    ...
    ac.id_funcionario,
    (((p.per_nombre)::text || ' '::text) || (p.per_apellido)::text) AS funcionario,
    to_char((ac.vac_fecha_ape)::timestamp with time zone, 'DD/MM/YYYY HH24:MI:ss'::text) AS fech_apertura,
    to_char((ac.vac_fecha_cie)::timestamp with time zone, 'DD/MM/YYYY HH24:MI:ss'::text) AS fech_cierre,
    ac.vac_monto_efec AS sistema_efectivo,
    ac.vac_monto_cheq AS sistema_cheque,
    ac.vac_monto_tarj AS sistema_tarjeta,
    ac.vac_monto_tranf AS sistema_tranf,
    (((ac.vac_monto_efec + ac.vac_monto_cheq) + ac.vac_monto_tarj) + ac.vac_monto_tranf) AS sistema_total,
    arq.arq_monto_efec AS fisico_efectivo,
    arq.arq_monto_cheq AS fisico_cheque,
    arq.arq_monto_tarj AS fisico_tarjeta,
    arq.arq_monto_tranf AS fisico_tranf,
    (((arq.arq_monto_efec + arq.arq_monto_cheq) + arq.arq_monto_tarj) + arq.arq_monto_tranf) AS fisico_total,
    (arq.arq_monto_efec - ac.vac_monto_efec) AS dif_efectivo,
    (arq.arq_monto_cheq - ac.vac_monto_cheq) AS dif_cheque,
    (arq.arq_monto_tarj - ac.vac_monto_tarj) AS dif_tarjeta,
    (arq.arq_monto_tranf - ac.vac_monto_tranf) AS dif_transfe,
    ((((arq.arq_monto_efec + arq.arq_monto_cheq) + arq.arq_monto_tarj) + arq.arq_monto_tranf) - (((ac.vac_monto_efec + ac.vac_monto_cheq) + ac.vac_monto_tarj) + ac.vac_monto_tranf)) AS dif_total,
    to_char((arq.arq_fecha)::timestamp with time zone, 'DD/MM/YYYY HH24:MI:ss'::text) AS fecha_arquo,
    arq.estado AS estado_arqueo,
    ac.estado AS estado_apertura
   FROM (((((public.vent_aperturas_cierres ac
     LEFT JOIN public.vent_arqueo_control arq ON (((ac.id_vac = arq.id_vac) AND (ac.id_caja = arq.id_caja) AND (ac.id_sucursal = arq.id_sucursal))))
     LEFT JOIN public.cajas c ON ((ac.id_caja = c.id_caja)))
     LEFT JOIN public.sucursales s ON ((s.id_sucursal = ac.id_sucursal)))
     LEFT JOIN public.funcionarios f ON ((ac.id_funcionario = f.id_funcionario)))
     LEFT JOIN public.personas p ON ((p.id_persona = f.id_persona)));


ALTER TABLE public.v_arqueo_resumen OWNER TO postgres;
...
CREATE VIEW public.v_compras_detalles AS
 SELECT DISTINCT ON (cd.id_cc, cd.id_item, cd.id_sucursal) cd.id_cc,
    cd.id_item,
    cd.cantidad,
    cd.precio,
    cd.id_sucursal,
   ...
   FROM ((((((public.compras_detalle cd
     ...
     JOIN public.deposito de ON ((de.id_sucursal = cd.id_sucursal)));
 ...
 SELECT c.id_cc,
    ...
    ((((sum(
        ...
        CASE
            ...
            ELSE 0
        END)) AS sumatotal
   FROM ( SELECT DISTINCT ON (d.id_cc, d.id_item, d.id_sucursal) d.id_cc,
            d.id_item,
            d.cantidad,
            s.stock_cantidad,
            d.precio,
...
CREATE VIEW public.v_ventas_det AS
 SELECT DISTINCT ON (cd.id_vc, cd.id_item, cd.id_sucursal) cd.id_vc,
    cd.id_item,
    cd.cantidad,
    cd.precio,
    cd.auditoria,
   ...
   FROM ((((((public.ventas_det cd
     ...
     JOIN public.tipos_impuestos ti2 ON ((ti2.id_tip_impuesto = i.id_tip_impuesto)))
     JOIN public.marcas m ON ((m.id_mar = i.id_mar)))
     LEFT JOIN public.stock s ON ((s.id_item = cd.id_item)))
     LEFT JOIN public.deposito de ON ((de.id_sucursal = cd.id_sucursal)));
 ...
 SELECT c.id_vc,
    ...
    ((((sum(
        ...
        CASE
            ...
            ELSE 0
        END)) AS sumatotal
   FROM ( SELECT DISTINCT ON (d.id_vc, d.id_item, d.id_sucursal) d.id_vc,
            d.id_item,
            d.cantidad,
            s.stock_cantidad,
            d.precio,
...
--
-- Name: v_clientes_estados_salud; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_clientes_estados_salud AS
 SELECT c.id_cliente,
    es.id_es,
    es.es_nombre,
    es.es_descrip,
    ces.estado
   FROM ((public.clientes_estados_salud ces
     JOIN public.clientes c ON ((c.id_cliente = ces.id_cliente)))
     JOIN public.estados_salud es ON ((es.id_es = ces.id_es)));


ALTER TABLE public.v_clientes_estados_salud OWNER TO postgres;

--
-- Name: v_clientes_r; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_clientes_r AS
 SELECT c.id_cliente,
    c.id_persona,
    c.estado,
    c.auditoria,
   ...
   FROM ((((public.clientes c
     ...
     LEFT JOIN public.ciudades cd ON ((cd.id_ciudad = p.id_ciudad)))
     LEFT JOIN public.estado_civiles ec ON ((ec.id_ecivil = p.id_ecivil)))
     LEFT JOIN public.generos g ON ((g.id_genero = p.id_genero)));


ALTER TABLE public.v_clientes_r OWNER TO postgres;

--
-- Name: v_cobros_cheques; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_cobros_cheques AS
 SELECT cd.id_che,
    cd.che_nro_cheque,
    cd.che_vencimiento,
    cd.che_monto,
    ...
    cd.estado,
    cd.auditoria,
    to_char((cd.che_vencimiento)::timestamp with time zone, 'DD/MM/YYYY'::text) AS fecha_vencimiento,
    f.ee_razon_social,
    f.ee_tipo_entidad
   FROM (public.cobros_cheques cd
     JOIN public.entidades_emisoras f ON ((f.id_ee = cd.id_ee)));


ALTER TABLE public.v_cobros_cheques OWNER TO postgres;

--
-- Name: v_cobros_tarjetas; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_cobros_tarjetas AS
 SELECT DISTINCT ON (cd.id_ctar) cd.id_ctar,
    cd.tar_nro_tarjeta,
    cd.tar_vencimiento,
    cd.tar_monto,
    cd.id_cob,
    ...
    cd.estado,
    cd.auditoria,
    to_char((cd.tar_vencimiento)::timestamp with time zone, 'DD/MM/YYYY'::text) AS fecha_vencimiento,
    ee.ee_razon_social,
    ee.ee_tipo_entidad,
    mt.mt_descrip
   FROM (((public.cobros_tarjetas cd
     LEFT JOIN public.entidades_adheridas f ON ((f.id_ee = cd.id_ee)))
     LEFT JOIN public.entidades_emisoras ee ON ((ee.id_ee = f.id_ee)))
     LEFT JOIN public.marcas_tarjetas mt ON ((mt.id_mt = f.id_mt)));


ALTER TABLE public.v_cobros_tarjetas OWNER TO postgres;

--
-- Name: v_cobros_transferencia; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_cobros_transferencia AS
 SELECT cd.id_ctra,
    cd.tra_nro_cuenta,
    cd.tra_fecha,
    cd.tra_monto,
    ...
    cd.estado,
    cd.auditoria,
    to_char((cd.tra_fecha)::timestamp with time zone, 'DD/MM/YYYY'::text) AS fecha,
    eeo.ee_razon_social AS rason_social_ori,
    eeo.ee_tipo_entidad AS tipo_entidad_ori,
    eed.ee_razon_social AS rason_social_des,
    eed.ee_tipo_entidad AS tipo_entidad_des
   FROM ((public.cobros_transferencia cd
     JOIN public.entidades_emisoras eeo ON ((eeo.id_ee = cd.id_ee_ori)))
     JOIN public.entidades_emisoras eed ON ((eed.id_ee = cd.id_ee_des)));


ALTER TABLE public.v_cobros_transferencia OWNER TO postgres;
...
CREATE VIEW public.v_comp_nota_cab AS
 SELECT cnc.id_not,
   ...
   FROM ((((((((public.comp_nota_cab cnc
     ...
     JOIN public.empresas e ON ((e.id_empresa = s.id_empresa)))
     LEFT JOIN public.tipos_movimientos tm ON ((tm.id_tm = cnc.id_tm)));
...
CREATE VIEW public.v_comp_transfers_det AS
 SELECT DISTINCT ON (cd.id_tra, cd.id_item, cd.id_sucursal) cd.id_tra,
    cd.id_item,
    cd.cantidad,
    cd.auditoria,
    cd.id_sucursal,
   ...
   FROM (((((public.comp_transfers_det cd
     ...
     LEFT JOIN public.tipos_impuestos ti2 ON ((ti2.id_tip_impuesto = i.id_tip_impuesto)))
     LEFT JOIN public.marcas m ON ((m.id_mar = i.id_mar)))
     LEFT JOIN public.stock s ON ((s.id_item = i.id_item)));
...
CREATE VIEW public.v_compras_cab AS
 SELECT cc.id_cc,
    ...
    (((pf.per_nombre)::text || ' '::text) || (pf.per_apellido)::text) AS funcionario,
    ( SELECT COALESCE(sum((compras_detalle.cantidad * compras_detalle.precio)), (0)::bigint) AS "coalesce"
   ...
   FROM ((((((public.compras_cabecera cc
     ...
     LEFT JOIN public.proveedores p2 ON ((p2.id_proveedor = cc.id_proveedor)))
     LEFT JOIN public.personas p ON ((p.id_persona = p2.id_persona)))
     LEFT JOIN public.funcionarios f ON ((f.id_funcionario = cc.id_funcionario)))
     LEFT JOIN public.personas pf ON ((pf.id_persona = f.id_persona)))
...
--
-- Name: v_entidades_adheridas; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_entidades_adheridas AS
 SELECT ea.id_ee,
    ea.id_mt,
    ee.ee_razon_social,
    ee.ee_tipo_entidad,
   ...
   FROM ((public.entidades_adheridas ea
     JOIN public.entidades_emisoras ee ON ((ee.id_ee = ea.id_ee)))
     JOIN public.marcas_tarjetas mt ON ((mt.id_mt = ea.id_mt)));


ALTER TABLE public.v_entidades_adheridas OWNER TO postgres;

--
-- Name: v_funcionarios_dni; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_funcionarios_dni AS
 SELECT f.id_funcionario,
    ...
    f.id_persona,
    f.id_cargo,
    (((((p.per_nombre)::text || ' '::text) || (p.per_apellido)::text) || ' - '::text) || (p.per_ci)::text) AS funcionario_ci,
    (((((p.per_nombre)::text || ' '::text) || (p.per_apellido)::text) || ' - '::text) || (p.per_ruc)::text) AS funcionario_ruc
   FROM (public.funcionarios f
     JOIN public.personas p ON ((p.id_persona = f.id_persona)));


ALTER TABLE public.v_funcionarios_dni OWNER TO postgres;
...
CREATE VIEW public.v_items AS
 SELECT i.id_item,
   ...
   FROM ((((public.items i
     LEFT JOIN public.marcas m ON ((m.id_mar = i.id_mar)))
     LEFT JOIN public.tipos_items ti ON ((ti.id_tip_item = i.id_tip_item)))
     LEFT JOIN public.tipos_impuestos t2 ON ((t2.id_tip_impuesto = i.id_tip_impuesto)))
     LEFT JOIN public.stock s ON ((s.id_item = i.id_item)));
...
--
-- Name: v_items_conceptos; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_items_conceptos AS
 SELECT i.id_item,
    i.item_descrip,
    i.precio_compra,
    i.precio_venta,
...
ALTER TABLE public.v_items_conceptos OWNER TO postgres;
...
CREATE VIEW public.v_planes_servicios AS
 SELECT ps.id_plan_servi,
    ...
    ti.tip_imp_tasa,
    ti.tip_imp_tasa2,
    COALESCE(((ps.precio_servicio)::numeric * ((1)::numeric - (p.pro_valor / (100)::numeric))), (ps.precio_servicio)::numeric) AS precio_con_promocion,
    p.id_promo,
    p.pro_nombre,
    p.pro_valor AS porcentaje_descuento,
    o.td_descrip,
    o.td_duracion
   FROM (((public.planes_servicios ps
     JOIN public.tipos_impuestos ti ON ((ti.id_tip_impuesto = ps.id_tip_impuesto)))
     LEFT JOIN public.tipos_duracion o ON ((o.id_td = ps.id_td)))
     LEFT JOIN public.promociones p ON (((p.id_promo = ps.id_promo) AND (p.estado = 'ACTIVO'::public.estados) AND (CURRENT_DATE >= p.pro_fecha_ini) AND (CURRENT_DATE <= p.pro_fecha_fin))));
...
CREATE VIEW public.v_serv_alimentaciones_det AS
 SELECT md.id_ali,
   ...
   FROM ((public.serv_alimentaciones_det md
     ...
     JOIN public.recetas re ON ((re.id_res = md.id_res)));
...
CREATE VIEW public.v_serv_inscripciones_membresias AS
 SELECT sim.id_mem,
   ...
   FROM ((((public.serv_inscripciones_membresias sim
     LEFT JOIN public.planes_servicios ps ON ((ps.id_plan_servi = sim.id_plan_servi)))
     LEFT JOIN public.promociones p ON ((p.id_promo = ps.id_promo)))
     LEFT JOIN public.tipos_duracion td ON ((td.id_td = ps.id_td)))
     LEFT JOIN public.tipos_impuestos ti ON ((ti.id_tip_impuesto = ps.id_tip_impuesto)));
...
CREATE VIEW public.v_serv_membresias_det AS
 SELECT smd.id_mem,
    ...
    td.td_duracion
   FROM ((((public.serv_membresias_det smd
     LEFT JOIN public.planes_servicios ps ON ((ps.id_plan_servi = smd.id_plan_servi)))
     LEFT JOIN public.promociones p ON ((p.id_promo = ps.id_promo)))
     LEFT JOIN public.tipos_duracion td ON ((td.id_td = ps.id_td)))
     LEFT JOIN public.tipos_impuestos ti ON ((ti.id_tip_impuesto = ps.id_tip_impuesto)));
...
CREATE VIEW public.v_servicios_inscripciones_detalle AS
 SELECT sid.id_inscrip,
   ...
   FROM ((((public.servicios_inscripciones_detalle sid
     LEFT JOIN public.planes_servicios ps ON ((ps.id_plan_servi = sid.id_plan_servi)))
     LEFT JOIN public.promociones p ON ((p.id_promo = ps.id_promo)))
     LEFT JOIN public.tipos_duracion td ON ((td.id_td = ps.id_td)))
     LEFT JOIN public.tipos_impuestos ti ON ((ti.id_tip_impuesto = ps.id_tip_impuesto)));
...
CREATE VIEW public.v_vehiculos AS
 SELECT v.id_vehiculo,
    ...
    v.veh_chapa,
    v.veh_color,
    (((((((v.veh_chapa)::text || ' - '::text) || v.veh_descrip) || ' - '::text) || (v.veh_color)::text) || ' - '::text) || (m.mar_descrip)::text) AS vehiculo,
    m.mar_descrip
...
--
-- Name: vent_cobros_cab; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.vent_cobros_cab (
    id_cob integer NOT NULL,
    id_vac integer,
    id_caja integer,
    id_sucursal integer,
...
ALTER TABLE public.vent_cobros_cab OWNER TO postgres;
...
--
-- Name: v_vent_cobros_cab; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_vent_cobros_cab AS
 SELECT cnc.id_cob,
    cnc.id_vac,
    cnc.id_caja,
    cnc.id_sucursal,
    ...
    cnc.auditoria,
    cnc.id_vc,
    to_char((cnc.cob_fecha)::timestamp with time zone, 'DD/MM/YYYY'::text) AS fecha,
    s.suc_nombre,
    s.suc_direccion,
    s.suc_correo,
    s.suc_telefono,
    ...
    cc.caj_descrip,
    vc.vc_nro_factura
   FROM ((((((public.vent_cobros_cab cnc
     JOIN public.ventas_cab vc ON ((vc.id_vc = cnc.id_vc)))
     JOIN public.cajas cc ON ((cc.id_caja = cnc.id_caja)))
     JOIN public.sucursales s ON ((s.id_sucursal = cnc.id_sucursal)))
     JOIN public.funcionarios f ON ((f.id_funcionario = cnc.id_funcionario)))
     JOIN public.personas func ON ((func.id_persona = f.id_persona)))
     JOIN public.empresas e ON ((e.id_empresa = s.id_empresa)));
...
ALTER TABLE public.v_vent_cobros_cab OWNER TO postgres;

--
-- Name: vent_cobros_det; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.vent_cobros_det (
    id_cob integer NOT NULL,
    id_cue integer NOT NULL,
    id_fc integer,
...
ALTER TABLE public.vent_cobros_det OWNER TO postgres;
...
--
-- Name: v_vent_cobros_det; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_vent_cobros_det AS
 SELECT cd.id_cob,
    cd.id_cue,
    cd.id_fc,
    cd.cob_monto_efe,
    ...
    v.cue_saldo,
    v.nro_cuota,
    ('COBRO DE CUOTA: '::text || v.nro_cuota) AS cue_concepto
   FROM ((public.vent_cobros_det cd
     LEFT JOIN public.formas_cobros f ON ((f.id_fc = cd.id_fc)))
     LEFT JOIN public.vent_cuentas_cobrar v ON ((v.id_cue = cd.id_cue)));


ALTER TABLE public.v_vent_cobros_det OWNER TO postgres;

--
-- Name: v_vent_cobros_montos; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_vent_cobros_montos AS
 SELECT cnc.id_cob,
    cnc.id_vac,
    cnc.id_vc,
    cnc.id_caja,
    ...
    sum(
        CASE
            WHEN (cd.id_cob IS NOT NULL) THEN cd.cob_monto_efe
            ELSE (0)::numeric
        END) AS total_efectivo,
    sum(
        CASE
            WHEN (ccq.id_cob IS NOT NULL) THEN ccq.che_monto
            ELSE (0)::numeric
        END) AS total_cheque,
    sum(
        CASE
            WHEN (ct.id_cob IS NOT NULL) THEN ct.tar_monto
            ELSE (0)::numeric
        END) AS total_tarjeta,
    sum(
        CASE
            WHEN (ctr.id_cob IS NOT NULL) THEN ctr.tra_monto
            ELSE (0)::numeric
        END) AS total_transfe,
    (((sum(
        CASE
            WHEN (cd.id_cob IS NOT NULL) THEN cd.cob_monto_efe
            ELSE (0)::numeric
   ...
   FROM ((((((((((public.vent_cobros_cab cnc
     LEFT JOIN public.ventas_cab vc ON ((vc.id_vc = cnc.id_vc)))
     LEFT JOIN public.vent_cobros_det cd ON ((cnc.id_cob = cd.id_cob)))
     LEFT JOIN public.cobros_cheques ccq ON ((cnc.id_cob = ccq.id_cob)))
     LEFT JOIN public.cobros_tarjetas ct ON ((cnc.id_cob = ct.id_cob)))
     LEFT JOIN public.cobros_transferencia ctr ON ((cnc.id_cob = ctr.id_cob)))
     LEFT JOIN public.cajas cc ON ((cc.id_caja = cnc.id_caja)))
     LEFT JOIN public.sucursales s ON ((s.id_sucursal = cnc.id_sucursal)))
     LEFT JOIN public.funcionarios f ON ((f.id_funcionario = cnc.id_funcionario)))
     LEFT JOIN public.personas func ON ((func.id_persona = f.id_persona)))
     LEFT JOIN public.empresas e ON ((e.id_empresa = s.id_empresa)))
  GROUP BY cnc.id_cob, cnc.id_vac, cnc.id_vc, cnc.id_caja, cnc.id_sucursal, cnc.id_funcionario, cnc.cob_fecha, cnc.estado, cnc.auditoria, s.suc_nombre, s.suc_direccion, s.suc_correo, s.suc_telefono, s.suc_imagen, func.per_nombre, func.per_apellido, e.emp_ruc, e.emp_denominacion, cc.caj_descrip, vc.vc_nro_factura;


ALTER TABLE public.v_vent_cobros_montos OWNER TO postgres;
...
CREATE TABLE public.vent_nota_cab (
    ...
    id_tm integer,
    id_tim integer,
    not_nro_documento character varying(50),
    id_vehiculo integer,
    id_chofer integer
);
...
CREATE VIEW public.v_vent_nota_cab AS
 SELECT cnc.id_not,
    ...
    cnc.id_vehiculo,
    cnc.id_chofer,
    to_char((cnc.not_fecha_elab)::timestamp with time zone, 'YYYY-MM-DD'::text) AS f_sin_hora,
    ...
    (((cli.per_nombre)::text || ' '::text) || (cli.per_apellido)::text) AS cliente,
    (((func.per_nombre)::text || ' '::text) || (func.per_apellido)::text) AS funcionario,
    (((((pc.per_nombre)::text || ' '::text) || (pc.per_apellido)::text) || ' '::text) || (pc.per_ruc)::text) AS chofer,
    e.emp_ruc,
    e.emp_denominacion,
    cc.vc_nro_factura,
    cc.vc_cuota,
    ...
    vv.vehiculo,
    cli.per_ruc,
    ( SELECT COALESCE(sum((vent_nota_det.cantidad * vent_nota_det.precio)), ((0)::bigint)::numeric) AS "coalesce"
           FROM public.vent_nota_det
          WHERE (vent_nota_det.id_not = cnc.id_not)) AS monto_detalles
   FROM ((((((((((((public.vent_nota_cab cnc
     LEFT JOIN public.ventas_cab cc ON ((cc.id_vc = cnc.id_vc)))
     LEFT JOIN public.sucursales s ON ((s.id_sucursal = cnc.id_sucursal)))
     LEFT JOIN public.funcionarios f ON ((f.id_funcionario = cnc.id_funcionario)))
     LEFT JOIN public.personas func ON ((func.id_persona = f.id_persona)))
     LEFT JOIN public.clientes c ON ((c.id_cliente = cnc.id_cliente)))
     ...
     LEFT JOIN public.empresas e ON ((e.id_empresa = s.id_empresa)))
     LEFT JOIN public.tipos_movimientos tm ON ((tm.id_tm = cnc.id_tm)))
     LEFT JOIN public.v_timbrados t ON ((t.id_tim = cnc.id_tim)))
     LEFT JOIN public.v_vehiculos vv ON ((vv.id_vehiculo = cnc.id_vehiculo)))
     LEFT JOIN public.funcionarios fc ON ((fc.id_funcionario = cnc.id_chofer)))
     LEFT JOIN public.personas pc ON ((pc.id_persona = fc.id_persona)));
...
CREATE VIEW public.v_vent_nota_det AS
 SELECT cd.id_not,
   ...
   FROM ((((((public.vent_nota_det cd
     ...
     LEFT JOIN public.marcas m ON ((m.id_mar = i.id_mar)))
     LEFT JOIN public.stock s ON ((s.id_item = i.id_item)))
  GROUP BY cd.id_not, cd.id_item, cd.cantidad, cd.precio, cd.monto, i.item_descrip, i.id_mar, i.id_tip_item, i.item_concepto, ti2.id_tip_impuesto, m.mar_descrip, ti.tip_item_descrip, s.id_sucursal, s.stock_cantidad;
...
CREATE VIEW public.v_ventas_cab AS
 SELECT cc.id_vc,
   ...
   FROM (((((((((public.ventas_cab cc
     ...
     LEFT JOIN public.tipos_movimientos tm ON ((tm.id_tm = cc.id_tm)));
...
CREATE VIEW public.v_ventas_consolidacion AS
 SELECT c.id_vc,
   ...
   FROM ( SELECT DISTINCT ON (d.id_vc, d.id_item, d.id_sucursal) d.id_vc,
           ...
           FROM ((public.v_ventas_det d
             JOIN public.stock s ON ((d.id_stock = s.id_item)))
             JOIN public.deposito p ON ((p.id_sucursal = d.id_sucursal)))
        UNION ALL
         SELECT p.id_vc,
            p.id_item,
            p.cantidad,
...
--
-- Name: ventas_servicio_det; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.ventas_servicio_det (
    id_vc integer NOT NULL,
    id_cm integer NOT NULL,
    nro_cuota integer,
    precio integer,
...
ALTER TABLE public.ventas_servicio_det OWNER TO postgres;

--
-- Name: v_ventas_servicio_det; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_ventas_servicio_det AS
 SELECT vsd.id_vc,
    vsd.id_cm,
    vsd.nro_cuota,
    vsd.precio,
    vsd.auditoria,
    ('FACTURACION DE LA CUOTA: '::text || vsd.nro_cuota) AS concepto,
    c.cliente,
    cm.cm_fecha_venc,
    cm.cm_dni,
    to_char((cm.cm_fecha_venc)::timestamp with time zone, 'DD/MM/YYYY'::text) AS fecha_vence
   FROM ((public.ventas_servicio_det vsd
     LEFT JOIN public.cuotas_membresias cm ON ((cm.id_cm = vsd.id_cm)))
     LEFT JOIN public.v_clientes c ON ((c.id_cliente = cm.id_cliente)));


ALTER TABLE public.v_ventas_servicio_det OWNER TO postgres;
...
COPY public.accesos (id_acceso, id_usuario, estado, auditoria, acc_login, acc_contrasena, acc_ip, acc_fecha) FROM stdin;
11	\N	ACTIVO	\N	admin	2312431234	::1	2025-10-15 20:44:52
12	\N	ACTIVO	\N	admin	4123412	::1	2025-10-15 20:45:10
13	1	ACTIVO	INSERCION/admin/2025-10-15 17:45:29.28706-03	admin	123	::1	2025-10-15 20:45:29
14	1	ACTIVO	INSERCION/admin/2025-10-15 17:54:02.586618-03	admin	123	::1	2025-10-15 20:54:02
15	1	ACTIVO	INSERCION/admin/2025-10-15 23:02:51.468185-03	admin	123	::1	2025-10-16 02:02:51
16	1	ACTIVO	INSERCION/admin/2025-10-16 18:19:23.835976-03	admin	123	::1	2025-10-16 21:19:23
17	1	ACTIVO	INSERCION/admin/2025-10-16 18:23:39.78579-03	admin	123	::1	2025-10-16 21:23:39
18	1	ACTIVO	INSERCION/admin/2025-10-17 14:00:45.595532-03	admin	123	::1	2025-10-17 17:00:45
19	1	ACTIVO	INSERCION/admin/2025-10-18 17:34:22.760397-03	admin	123	::1	2025-10-18 20:34:22
20	1	ACTIVO	INSERCION/admin/2025-10-19 17:12:14.191852-03	admin	123	::1	2025-10-19 20:12:14
\.
...
8		INACTIVO	\N	1
7	JNK	INACTIVO	\N	1
9	CAAGUAZÚ	ACTIVO	\N	1
10	CAPIATÁ	ACTIVO	\N	1
11	LUQUE	ACTIVO	\N	1
\.
...
2	3	ACTIVO	\N
3	9	ACTIVO	\N
1	5	ACTIVO	\rINACTIVACIONadmin/2025-10-03 21:08:34.589796-04\rACTIVACIONadmin/2025-10-03 21:08:35.667285-04\rINACTIVACIONadmin/2025-10-03 21:08:37.224546-04\rACTIVACIONadmin/2025-10-03 21:08:39.00736-04\rINACTIVACIONadmin/2025-10-03 21:08:40.397254-04\rACTIVACIONadmin/2025-10-03 21:08:41.795019-04
4	12	ACTIVO	INSERCION/admin/2025-10-03 21:56:55.242095-04
\.


--
-- Data for Name: clientes_estados_salud; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.clientes_estados_salud (id_cliente, id_es, estado, auditoria) FROM stdin;
1	1	ACTIVO	\N
1	4	ACTIVO	\N
2	2	ACTIVO	\N
3	10	ACTIVO	\N
2	9	ACTIVO	INSERCION/admin/2025-09-11 21:32:16.625181-04
2	1	ACTIVO	INSERCION/admin/2025-09-13 07:39:44.377942-04
4	3	ACTIVO	INSERCION/admin/2025-10-12 22:09:58.618977-03
\.


--
-- Data for Name: cobros_cheques; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.cobros_cheques (id_che, che_nro_cheque, che_vencimiento, che_monto, che_tipo_cheque, id_cob, id_ee, estado, auditoria) FROM stdin;
1	342423	2025-08-26	21417	CHEQUE COMÚN	1	1	ACTIVO	INSERCION/admin/2025-08-25 21:20:00.726974-04
2	342423	2025-09-30	50000	CHEQUE COMÚN	4	1	ACTIVO	INSERCION/admin/2025-09-20 07:51:58.633843-04
3	342423	2025-09-05	-1	CHEQUE COMÚN	6	1	ACTIVO	INSERCION/admin/2025-09-22 19:57:03.112235-04
4	342423	2025-10-16	5000	CHEQUE A FECHA O DIFERIDO	8	1	ACTIVO	INSERCION/admin/2025-10-16 19:57:09.062259-03
\.


--
-- Data for Name: cobros_tarjetas; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.cobros_tarjetas (id_ctar, tar_nro_tarjeta, tar_vencimiento, tar_monto, id_cob, id_ee, id_mt, estado, auditoria) FROM stdin;
1	1234567812345678	2025-08-29	21417	1	3	2	ACTIVO	INSERCION/admin/2025-08-29 18:41:14.156078-04
2	123412341233	2025-09-20	15000	4	4	1	ACTIVO	INSERCION/admin/2025-09-19 22:19:48.006253-04
3	123412341233	2025-09-20	50000	4	4	1	ACTIVO	INSERCION/admin/2025-09-20 07:51:09.636882-04
4	123412341233	2025-09-22	200000	6	1	1	ACTIVO	INSERCION/admin/2025-09-22 19:58:40.98086-04
5	1234567812345678	2025-10-16	700	8	3	2	ACTIVO	INSERCION/admin/2025-10-16 20:12:16.727009-03
\.


--
-- Data for Name: cobros_transferencia; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.cobros_transferencia (id_ctra, tra_nro_cuenta, tra_fecha, tra_monto, tra_motivo, id_cob, id_ee_ori, id_ee_des, estado, auditoria) FROM stdin;
1	43-43535	2025-08-29	21417	PAGO DE FACTURA	1	1	1	ACTIVO	INSERCION/admin/2025-08-29 18:34:47.767394-04
2	43-32343	2025-09-20	30000	PAGO DE FACTURA	4	1	2	ACTIVO	INSERCION/admin/2025-09-20 07:52:30.713427-04
3	43-32343	2025-10-16	5000	PAGO DE FACTURA	8	2	2	ACTIVO	INSERCION/admin/2025-10-16 20:08:24.18054-03
\.
...
1	2024-12-30	1	1	CONFIRMADO	INSERCION/admin/2024-12-29 23:33:47.063025-03\rCONFIRMACION/admin/2025-09-05 21:34:26.847276-04	N/A
2	2025-09-06	1	1	CONFIRMADO	INSERCION/admin/2025-09-06 08:22:21.525009-04\rCONFIRMACION/admin/2025-09-06 08:25:31.646335-04	N/A
3	2025-09-07	1	1	PENDIENTE	INSERCION/admin/2025-09-07 15:30:15.6544-04	n/A
\.
...
COPY public.comp_ajustes_det (id_caju, id_item, id_motivo, cantidad, precio, auditoria, id_sucursal) FROM stdin;
1	2	14	3	250000	INSERCION/admin/2025-09-05 21:34:06.428442-04	\N
2	6	6	3	10000	INSERCION/admin/2025-09-06 08:25:27.344107-04	\N
3	6	9	3	10000	INSERCION/admin/2025-09-07 15:40:15.685702-04	\N
3	2	8	2	250000	INSERCION/admin/2025-10-04 17:49:19.49216-04	1
\.
...
2	2025-09-06	9000	9000	ACTIVO	INSERCION/admin/2025-09-05 21:28:33.57829-04
3	2025-09-06	81544000	81544000	ACTIVO	INSERCION/admin/2025-09-06 08:13:17.75452-04
4	2025-09-06	2000	2000	ACTIVO	INSERCION/admin/2025-09-06 08:14:52.441812-04
5	2025-09-07	2250000	2250000	ACTIVO	INSERCION/admin/2025-10-03 23:51:10.368963-04
\.
...
COPY public.comp_nota_cab (id_not, not_fecha, not_fecha_docu, not_tipo_nota, id_cc, id_sucursal, id_funcionario, id_proveedor, estado, auditoria, id_tm, not_vehiculo, not_chofer, not_nro_documento) FROM stdin;
2	2025-10-07	2025-10-07	REMISION	1	1	1	1	PENDIENTE	INSERCION/admin/2025-10-07 15:32:59.033464-03	13	n/a	n/a	n/a
1	2025-10-07	2025-10-07	CREDITO	1	1	1	1	PENDIENTE	INSERCION/admin/2025-10-06 22:50:04.475359-03	2	N/A	N/A	001-001-0000023
3	2025-10-07	2025-10-07	DEBITO	1	1	1	1	PENDIENTE	INSERCION/admin/2025-10-07 19:53:49.619466-03	9	N/A	N/A	n/a
\.
...
1	2025-08-30	2025-09-05	2025-09-06	1	1	1	2	1	2	1	2	N/A	CONFIRMADO	INSERCION/admin/2025-08-29 22:09:46.814921-04\rENVIADO/admin/2025-09-05 19:14:57.974207-04\rCONFIRMACION/admin/2025-09-05 21:35:16.8499-04
2	2025-09-06	2025-09-06	2025-09-06	1	1	1	2	1	2	2	3	N/A	CONFIRMADO	INSERCION/admin/2025-09-06 08:27:12.460592-04\rENVIADO/admin/2025-09-06 08:27:40.351241-04\rCONFIRMACION/admin/2025-09-06 08:28:47.758338-04
3	2025-09-06	2000-02-02	2000-02-02	1	1	2	1	2	1	2	2	n/a\n	ANULADO	INSERCION/admin/2025-09-06 08:29:08.325043-04\rANULACION/admin/2025-09-06 08:29:24.428972-04
4	2025-09-07	2025-10-05	2025-10-05	1	1	1	2	1	2	2	2	n/a	CONFIRMADO	INSERCION/admin/2025-09-07 15:43:15.083836-04\rENVIADO/admin/2025-10-04 06:40:15.78046-04\rCONFIRMACION/admin/2025-10-04 06:40:18.384245-04\rENVIADO/admin/2025-10-04 18:22:20.437334-04\rCONFIRMACION/admin/2025-10-04 18:22:24.786791-04\rENVIADO/admin/2025-10-04 19:34:56.429439-04\rCONFIRMACION/admin/2025-10-04 19:34:59.994539-04\rENVIADO/admin/2025-10-04 20:04:25.337425-04\rCONFIRMACION/admin/2025-10-04 20:04:26.676906-04\rENVIADO/admin/2025-10-04 22:15:05.070472-04\rCONFIRMACION/admin/2025-10-04 22:15:06.702849-04\rENVIADO/admin/2025-10-04 22:18:57.570707-04\rENVIADO/admin/2025-10-05 14:12:08.790254-03\rCONFIRMACION/admin/2025-10-05 14:12:12.620641-03
5	2025-10-05	2000-02-02	2000-02-02	1	1	2	1	2	1	1	2	n	PENDIENTE	INSERCION/admin/2025-10-05 20:52:16.926314-03
\.
...
COPY public.comp_transfers_det (id_tra, id_item, cantidad, auditoria, id_sucursal) FROM stdin;
1	2	2	INSERCION/admin/2025-08-29 22:15:29.945367-04	1
1	3	2	INSERCION/admin/2025-08-29 22:15:53.214872-04	1
2	2	25	INSERCION/admin/2025-09-06 08:27:28.352527-04	1
4	2	94	INSERCION/admin/2025-10-05 14:11:28.609159-03	2
5	5	3	INSERCION/admin/2025-10-10 22:03:38.963539-03	1
5	4	5	INSERCION/admin/2025-10-10 22:03:48.439429-03	1
\.
...
COPY public.compras_cabecera (id_cc, cc_fecha, cc_tipo_factura, cc_cuota, id_sucursal, id_funcionario, id_proveedor, estado, auditoria, cc_intervalo, cc_nro_factura, cc_timbrado, cc_fecha_emi) FROM stdin;
1	2024-04-30	CONTADO	0	1	1	1	CONFIRMADO	INSERCION/admin/2024-12-29 22:44:35.054273-03\rCONFIRMACION/admin/2024-12-29 23:15:34.865327-03	0	005-002-9876543	99887766	\N
2	2025-05-06	CREDITO	4	1	1	2	CONFIRMADO	INSERCION/admin/2025-09-05 21:25:52.991492-04\rMODIFICACION/admin/2025-09-05 21:26:07.848798-04\rMODIFICACION/admin/2025-09-05 21:27:05.728118-04\rMODIFICACION/admin/2025-09-05 21:27:22.149079-04\rCONFIRMACION/admin/2025-09-05 21:28:33.57829-04\rMODIFICACION/admin/2025-09-27 14:45:14.154775-04\rMODIFICACION/admin/2025-09-27 14:51:42.971077-04	15	012-003-0015345	02342334	2025-08-06
3	2025-06-06	CREDITO	4	1	1	2	CONFIRMADO	INSERCION/admin/2025-09-06 08:12:24.728864-04\rCONFIRMACION/admin/2025-09-06 08:13:17.75452-04\rMODIFICACION/admin/2025-09-27 14:51:31.684015-04	30	012-003-0015434	2131232	2025-09-26
4	2025-07-06	CREDITO	3	1	1	2	CONFIRMADO	INSERCION/admin/2025-09-06 08:14:13.12215-04\rCONFIRMACION/admin/2025-09-06 08:14:52.441812-04\rMODIFICACION/admin/2025-09-27 14:52:03.398335-04\rMODIFICACION/admin/2025-09-27 14:52:38.806908-04	30	012-003-0015321	423423	2025-09-26
5	2025-09-07	CONTADO	0	1	1	1	PENDIENTE	INSERCION/admin/2025-09-07 16:44:22.199672-04\rMODIFICACION/admin/2025-09-07 18:31:24.661118-04\rMODIFICACION/admin/2025-09-07 18:40:02.260038-04\rCONFIRMACION/admin/2025-10-03 23:51:10.368963-04	0	012-003-00153234	42342342	2025-09-30
\.
...
COPY public.compras_detalle (id_cc, id_item, cantidad, precio, id_sucursal) FROM stdin;
5	2	1	250000	1
5	3	1	300000	1
2	6	3	3000	1
4	6	1	2000	1
...
2	2025-04-06	CONTADO	0	1	1	1	CERRADO	INSERCION/admin/2025-04-06 14:14:16.49388-04\rCONFIRMACION/admin/2025-09-06 08:10:28.587336-04\rCERRAR/admin/2025-09-06 08:13:17.75452-04	0
3	2025-09-06	CREDITO	4	1	1	1	CERRADO	INSERCION/admin/2025-09-06 08:11:01.246639-04\rCONFIRMACION/admin/2025-09-06 08:11:37.169161-04\rCERRAR/admin/2025-09-06 08:14:52.441812-04	30
4	2025-09-07	CONTADO	0	1	1	1	PENDIENTE	INSERCION/admin/2025-09-06 20:34:35.387114-04	0
\.
...
3	2025-04-06	2025-04-06	1	1	ANULADO	INSERCION/admin/2025-04-06 14:01:16.77413-04\rANULACION/admin/2025-04-06 14:04:27.441079-04
2	2024-12-30	2024-12-30	1	1	CERRADO	INSERCION/admin/2024-12-30 15:59:49.437843-03\rCONFIRMACION/admin/2025-04-06 14:01:12.153047-04\rCERRAR/admin/2025-04-06 14:12:58.479235-04
4	2025-04-06	2025-04-17	1	1	ANULADO	INSERCION/admin/2025-04-06 14:05:18.761541-04\rANULACION/admin/2025-08-29 21:59:16.188751-04
5	2025-09-05	2025-09-30	1	1	CONFIRMADO	INSERCION/admin/2025-09-05 21:17:55.017304-04\rCONFIRMACION/admin/2025-09-05 21:18:11.819081-04
6	2025-09-06	2025-09-06	1	1	CONFIRMADO	INSERCION/admin/2025-09-06 08:00:59.866375-04\rCONFIRMACION/admin/2025-09-06 08:01:22.35188-04
7	2025-09-06	2025-09-06	1	1	CONFIRMADO	INSERCION/admin/2025-09-06 08:01:45.932035-04\rCONFIRMACION/admin/2025-09-06 08:05:17.992791-04\rCERRAR/admin/2025-09-06 08:08:48.410083-04\rCONFIRMACION/admin/2025-09-06 18:31:19.109231-04
8	2025-09-06	2025-10-10	1	1	CONFIRMADO	INSERCION/admin/2025-09-06 18:31:23.889761-04\rCONFIRMACION/admin/2025-10-10 16:18:23.268353-03
9	2025-10-10	2025-10-10	1	1	PENDIENTE	INSERCION/admin/2025-10-10 16:20:28.686679-03
\.
...
6	2025-09-06	2025-09-30	5325	N/A	1	1	1	CONFIRMADO	INSERCION/admin/2025-09-05 20:29:18.824298-04\rCONFIRMACION/admin/2025-09-06 08:05:58.17456-04
7	2025-09-06	2025-09-30	324422	N/A	1	1	2	CONFIRMADO	INSERCION/admin/2025-09-06 08:07:49.792014-04\rMODIFICACION/admin/2025-09-06 08:08:42.900526-04\rCONFIRMACION/admin/2025-09-06 08:08:48.410083-04
...
2	2024-12-30	2025-01-30	423	N/A	1	1	1	CERRADO	INSERCION/admin/2024-12-29 23:02:04.900303-03\rCONFIRMACION/admin/2024-12-29 23:02:59.693832-03\rCERRAR/admin/2025-09-06 08:10:28.587336-04
5	2025-04-06	2025-04-06	32	N/A	1	1	1	CERRADO	INSERCION/admin/2025-04-06 14:11:19.516757-04\rCONFIRMACION/admin/2025-04-06 14:12:58.479235-04\rCERRAR/admin/2025-09-06 08:11:37.169161-04
8	2025-09-06	2025-09-06	4234	n/a	1	1	1	PENDIENTE	INSERCION/admin/2025-09-06 18:29:00.558581-04
\.
...
--
-- Data for Name: cuotas_membresias; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.cuotas_membresias (id_cm, nro_cuota, cm_monto, cm_fecha_venc, cm_dni, id_mem, estado, auditoria, id_cliente) FROM stdin;
1	1	140000	2025-09-14	6789345	11	PAGADO	INSERCION/admin/2025-09-17 16:35:05.086866-04	1
2	1	370000	2025-09-21	6789345	12	PENDIENTE	INSERCION/admin/2025-09-20 13:50:49.440143-04	1
3	1	550000	2025-09-21	9823322	13	PENDIENTE	INSERCION/admin/2025-09-20 13:57:55.566108-04	3
4	1	480000	2025-09-23	3458987	14	PAGADO	INSERCION/admin/2025-09-22 19:49:27.922538-04	2
\.
...
COPY public.devoluciones (id_cob, id_cue, id_fc, dev_monto, auditoria) FROM stdin;
\.
...
1	80635801-2	ENERGYM	EUSEBIO AYALA	energym@gmail.com	0981123123	ENTRENAMIENTO FISICO	XXX-3214	ACTIVO	\N
\.


--
-- Data for Name: entidades_adheridas; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.entidades_adheridas (id_ee, id_mt) FROM stdin;
1	1
1	2
2	1
2	4
...
COPY public.entidades_emisoras (id_ee, ee_razon_social, ee_direccion, ee_telefono, ee_ruc, ee_email, ee_tipo_entidad, estado, auditoria) FROM stdin;
1	BANCO NACIONAL DE PARAGUAY	Av. Principal 123, Asunción	021 444 5678	800123456-1	contacto@bnp.com.py	BANCO	ACTIVO	INSERCION/admin/2025-02-28 21:13:59.769434-03
2	COOPERATIVA SAN ROQUE	Calle San Roque 456, Ciudad del Este	061 789 1234	801987654-2	info@coopsanroque.com.py	COOPERATIVA	ACTIVO	INSERCION/admin/2025-02-28 21:13:59.769434-03
3	FINANCIERA SOLIDARIA	Av. Mariscal López 789, Encarnación	071 333 5678	802654321-3	atencion@financierasolidaria.com.py	FINANCIERA	ACTIVO	INSERCION/admin/2025-02-28 21:13:59.769434-03
4	ASEGURADORA LA SEGURA	Ruta 1 Km 12, San Lorenzo	021 555 6789	803112233-4	soporte@lasegura.com.py	ASEGURADORA	ACTIVO	INSERCION/admin/2025-02-28 21:13:59.769434-03
5	CASA DE CAMBIO GLOBAL	Av. Internacional 101, Pedro Juan Caballero	0336 123 456	804998877-5	cambios@globalcambio.com.py	CASA DE CAMBIO	ACTIVO	INSERCION/admin/2025-02-28 21:13:59.769434-03
\.
...
--
-- Data for Name: estados_salud; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.estados_salud (id_es, es_nombre, es_descrip, estado, auditoria) FROM stdin;
1	HIPERTENSIÓN	Presión arterial elevada, controlar esfuerzos.	ACTIVO	\N
2	DIABETES	Controlar niveles de glucosa, evitar ayunos largos.	ACTIVO	\N
3	ASMA	Precaución con ejercicios de alta intensidad.	ACTIVO	\N
4	SOBREPESO	Ejercicios progresivos, evitar impacto excesivo.	ACTIVO	\N
5	OBESIDAD	Plan gradual, control de alimentación y esfuerzo.	ACTIVO	\N
6	LESIÓN DE RODILLA	Limitar ejercicios de impacto.	ACTIVO	\N
7	COLESTEROL ALTO	Promover dieta y ejercicios moderados.	ACTIVO	\N
8	EMBARAZO	Ejercicios de bajo impacto, previa autorización.	ACTIVO	\N
9	SEDENTARISMO	Inicio con rutinas ligeras.	ACTIVO	\N
10	SIN RESTRICCIONES	Apto para cualquier plan.	ACTIVO	\N
\.


--
-- Data for Name: formas_cobros; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.formas_cobros (id_fc, fc_descrip, estado, auditoria) FROM stdin;
1	CHEQUE	ACTIVO	\N
2	TARJETA	ACTIVO	\N
3	TRANSFERENCIA	ACTIVO	\N
4	EFECTIVO	ACTIVO	\N
\.
...
11	2023-04-22	\N	3000000	ACTIVO	\N	11	5
\.
...
COPY public.items (id_item, item_descrip, precio_compra, precio_venta, id_mar, id_tip_item, id_tip_impuesto, estado, auditoria, item_concepto) FROM stdin;
7	COSTO DE ENVÍO / FLETE	0	0	7	7	3	ACTIVO	INSERCION/admin/2024-10-20 20:30:46.976541-03	DEBITO
8	RECARGO POR CAMBIO DE CONDICIONES DE VENTA	0	0	7	7	3	ACTIVO	INSERCION/admin/2024-10-20 20:30:46.976541-03	DEBITO
13	INTERESES POR MORA	0	0	7	7	3	ACTIVO	INSERCION/admin/2024	DEBITO
11	CARGOS ADMINISTRATIVOS	0	0	7	7	3	ACTIVO	INSERCION/admin/2024	DEBITO
9	ERROR EN LA FACTURACIÓN	0	0	7	7	3	ACTIVO	INSERCION/admin/2024	CREDITO
10	BONIFICACIONES O DESCUENTOS	0	0	7	7	3	ACTIVO	INSERCION/admin/2024	CREDITO
12	PRODUCTOS DAÑADOS O VENCIDOS	0	0	7	7	3	ACTIVO	INSERCION/admin/2024	CREDITO
2	CREATINA	250000	280000	1	1	2	ACTIVO	INSERCION/admin/2023-10-20 20:30:46.976541-03	PRODUCTO
3	GUANTE DE GIMNACIO 	300000	350000	3	3	2	ACTIVO	INSERCION/admin/2023-10-20 20:30:46.976541-03	PRODUCTO
4	TOALLA	150000	170000	3	4	2	ACTIVO	INSERCION/admin/2023-10-20 20:30:46.976541-03	PRODUCTO
5	BEBIDA ENERGÉTICA	7000	9000	6	5	2	ACTIVO	INSERCION/admin/2023-10-20 20:30:46.976541-03	PRODUCTO
6	BARRA DE PROTEÍNA	10000	12500	1	6	2	ACTIVO	INSERCION/admin/2023-10-20 20:30:46.976541-03	PRODUCTO
1	PROTEÍNA	250000	300000	1	1	2	ACTIVO	INSERCION/admin/2023-10-20 20:30:46.976541-03\rINACTIVACIONadmin/2025-10-03 17:54:20.688419-04\rACTIVACIONadmin/2025-10-03 17:54:25.041407-04	PRODUCTO
14	NMMN	98898	89898	1	3	3	ACTIVO	INSERCION/admin/2025-10-04 07:33:33.427913-04	PRODUCTO
\.
...
2	0	818	0	ACTIVO	INSERCION/admin/2025-09-05 21:28:33.57829-04
3	0	7413090	0	ACTIVO	INSERCION/admin/2025-09-06 08:13:17.75452-04
4	0	181	0	ACTIVO	INSERCION/admin/2025-09-06 08:14:52.441812-04
5	0	204544	0	ACTIVO	INSERCION/admin/2025-10-03 23:51:10.368963-04
\.
...
3	0	160226	0	ACTIVO	INSERCION/admin/2025-02-24 21:46:59.181475-03
4	0	7636	0	ACTIVO	INSERCION/admin/2025-02-24 22:30:37.333658-03
5	0	22727	0	ACTIVO	INSERCION/admin/2025-02-26 15:06:42.457981-03
6	0	23363	0	ACTIVO	INSERCION/admin/2025-02-26 15:37:50.67759-03
7	0	636	0	ACTIVO	INSERCION/admin/2025-02-26 15:40:32.762475-03
9	0	27908	0	ACTIVO	INSERCION/admin/2025-08-25 20:49:37.62174-04
1	0	23363	0	ACTIVO	INSERCION/admin/2025-08-25 21:00:28.281185-04
2	0	113636	0	ACTIVO	INSERCION/admin/2025-08-28 20:58:29.551761-04
10	0	49999	0	ACTIVO	INSERCION/admin/2025-09-14 18:41:54.229148-04
11	0	0	0	ACTIVO	INSERCION/admin/2025-09-17 22:42:42.402827-04
12	0	22727	0	ACTIVO	INSERCION/admin/2025-09-17 23:15:14.450199-04
13	0	1818	0	ACTIVO	INSERCION/admin/2025-09-20 07:30:12.719425-04
14	0	23999	0	ACTIVO	INSERCION/admin/2025-09-20 07:41:35.623212-04
16	0	0	0	ACTIVO	INSERCION/admin/2025-09-22 19:54:16.748164-04
\.
...
8	JMC	ACTIVO	INSERCION/admin/2024-10-20 18:56:41.044767-03
9	CTO	ACTIVO	INSERCION/admin/2024-10-20 18:56:41.044767-03
7	CONCEPTO	ACTIVO	INSERCION/admin/2024-10-20 18:56:41.044767-03
\.


--
-- Data for Name: marcas_tarjetas; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.marcas_tarjetas (id_mt, mt_descrip, estado, auditoria) FROM stdin;
1	VISA	ACTIVO	INSERCION/admin/2025-05-09 19:05:04.397439-04
2	MASTERCARD	ACTIVO	INSERCION/admin/2025-05-09 19:05:04.397439-04
3	AMERICAN EXPRESS	ACTIVO	INSERCION/admin/2025-05-09 19:05:04.397439-04
4	CABAL	ACTIVO	INSERCION/admin/2025-05-09 19:05:04.397439-04
5	DINERS CLUB	ACTIVO	INSERCION/admin/2025-05-09 19:05:04.397439-04
\.
...
8	INFORMES VARIOS	fas fa-chart-pie	9	ACTIVO	\N
\.
...
COPY public.paginas (id_pagina, pag_descrip, pag_ubicacion, pag_icono, estado, auditoria, id_modulo, pag_orden) FROM stdin;
30	Accesos	/tesis/referenciales/accesos	far fa-circle	ACTIVO	\N	4	30
27	Clientes	/tesis/referenciales/clientes	far fa-circle	ACTIVO	\N	7	27
28	Plan de Servicio	/tesis/referenciales/plan_servicio	far fa-circle	ACTIVO	\N	6	28
29	Tipo de Impuesto	/tesis/referenciales/tipo_impuesto	far fa-circle	ACTIVO	\N	5	29
32	Inf. Servicios	/tesis/informes/inf_servicios	far fa-circle	ACTIVO	\N	8	32
33	Inf. Ventas	/tesis/informes/inf_ventas	far fa-circle	ACTIVO	\N	8	33
31	Inf. Compras	/tesis/informes/inf_compras	far fa-circle	ACTIVO	\N	8	31
21	Nota de Remisión	/tesis/venta/nota_remision	far fa-circle	ACTIVO	\N	3	22
35	Inf. Stock	/tesis/informes/inf_stock	far fa-circle	ACTIVO	\N	8	35
1	Pedido de Compra	/tesis/compra/pedidos	far fa-circle	ACTIVO	\N	1	1
2	Presupuesto de Proveedor	/tesis/compra/presupuesto	far fa-circle	ACTIVO	\N	1	2
3	Orden de Compra	/tesis/compra/orden	far fa-circle	ACTIVO	\N	1	3
4	Gestionar Compras	/tesis/compra/compras_facturacion	far fa-circle	ACTIVO	\N	1	4
5	Nota de Compra	/tesis/compra/nota_comp	far fa-circle	ACTIVO	\N	1	5
6	Ajuste de Stock	/tesis/compra/ajustes	far fa-circle	ACTIVO	\N	1	6
7	Transferencia	/tesis/compra/transferencia	far fa-circle	ACTIVO	\N	1	7
8	Inscripción	/tesis/servicio/inscripciones	far fa-circle	ACTIVO	\N	2	8
9	Rutinas	/tesis/servicio/rutina	far fa-circle	ACTIVO	\N	2	11
10	Mediciones	/tesis/servicio/medicion	far fa-circle	ACTIVO	\N	2	12
11	Plan de Alimentación	/tesis/servicio/plan_alimentario	far fa-circle	ACTIVO	\N	2	14
12	Evolución	/tesis/servicio/evolucion	far fa-circle	ACTIVO	\N	2	13
13	Presupuesto de Cliente	/tesis/servicio/presupuesto	far fa-circle	ACTIVO	\N	2	15
14	Asistencia	/tesis/servicio/asistencia	far fa-circle	ACTIVO	\N	2	10
15	Pedido de Clientes	/tesis/venta/pedidos_clientes	far fa-circle	ACTIVO	\N	3	16
16	Apertura y Cierre de Caja	/tesis/venta/apertura_cierre	far fa-circle	ACTIVO	\N	3	17
17	Gestionar Venta	/tesis/venta/ventas_facturacion	far fa-circle	ACTIVO	\N	3	18
18	Cobranza	/tesis/venta/cobranza	far fa-circle	ACTIVO	\N	3	19
19	Nota de Venta	/tesis/venta/nota_venta	far fa-circle	ACTIVO	\N	3	20
20	Arqueo de Caja	\N	far fa-circle	ACTIVO	\N	3	21
22	Paises	/tesis/referenciales/paises	far fa-circle	ACTIVO	\N	4	23
23	Ciudades	/tesis/referenciales/ciudades	far fa-circle	ACTIVO	\N	4	24
24	Personas	/tesis/referenciales/personas	far fa-circle	ACTIVO	\N	4	25
25	Generar Membresía	/tesis/servicio/membresia	far fa-circle	ACTIVO	\N	2	9
26	Proveedor	/tesis/referenciales/proveedor	far fa-circle	ACTIVO	\N	5	26
34	Items	/tesis/referenciales/item	far fa-circle	ACTIVO	\N	5	34
36	Inf. Cobros	/tesis/informes/inf_cobros	far fa-circle	ACTIVO	\N	8	36
\.
...
1	20	1	INACTIVO	INSERCION/admin/2023-10-16 17:06:59.625562-03
1	21	1	ACTIVO	INSERCION/admin/2023-10-16 17:06:59.625562-03
1	31	1	ACTIVO	INSERCION/admin/2023-10-18 09:56:19.682442-03
1	32	1	ACTIVO	INSERCION/admin/2023-10-18 09:56:19.682442-03
1	33	1	ACTIVO	INSERCION/admin/2023-10-18 09:56:19.682442-03
1	34	1	ACTIVO	INSERCION/admin/2023-10-18 09:56:19.682442-03
1	35	1	ACTIVO	INSERCION/admin/2023-10-18 09:56:19.682442-03
1	36	1	ACTIVO	INSERCION/admin/2023-10-18 09:56:19.682442-03
\.
...
12	IVAN	TORREZ	34534535	12345678-9	YYYY323X	ivantorrez@gmail.com	2006-02-17		t	ACTIVO	INSERCION/admin/2025-10-03 21:56:55.242095-04	1	1	1	\N
1	MATIAS	MARTINEZ	6321987-0	6321987	XXX-2312	matias5238klo@gmail.com	1995-11-04	0987234567	t	ACTIVO	INSERCION/admin/2023-10-31 16:59:03.174228-03\rINACTIVACIONadmin/2023-10-31 22:50:23.486386-03\rACTIVACIONadmin/2023-10-31 22:50:31.219195-03\rINACTIVACIONadmin/2024-12-25 23:28:11.779354-03\rACTIVACIONadmin/2024-12-25 23:28:29.326901-03	1	1	1	25
...
10	SDFAAASDF	FASFA	6321987-0\t	535345345	RWEREWRTWER	WTWETWE	2025-08-30		t	ACTIVO	INSERCION/admin/2025-08-30 07:15:41.206513-04	1	1	1	\N
11	CARLOS	GÓMEZ	8001234-5	4512367	Av. Mcal. López 123	carlos.gomez@mail.com	1990-05-12	0981123456	t	ACTIVO	\N	1	2	1	35
\.
...
COPY public.planes_servicios (id_plan_servi, ps_descrip, precio_servicio, id_tip_impuesto, estado, auditoria, id_promo, id_td) FROM stdin;
3	ENTRENAMIENTO PERSONALIZADO	50000	1	ACTIVO	\N	2	5
2	CROSSFIT 	70000	1	ACTIVO	\N	3	4
1	GIMNASIO LIBRE	200000	1	ACTIVO	\N	1	1
4	NUTRICION	300000	1	ACTIVO	\N	3	1
5	ENTRENAMIENTO DE FUERZA	250000	1	ACTIVO	\N	3	1
6	SEGUIMIENTO DE LA EVOLUCIÓN 	50000	1	ACTIVO	\N	3	5
7	SUMBA	180000	1	ACTIVO	\N	3	1
8	CARDIO	50000	1	ACTIVO	\N	3	1
\.
...
COPY public.promociones (id_promo, pro_nombre, pro_descrip, pro_valor, pro_fecha_ini, pro_fecha_fin, estado, auditoria) FROM stdin;
1	DESCUENTO DE BIENVENIDA	10% de descuento en el primer mes de inscripción.	10	2025-09-01	2025-09-30	ACTIVO	\N
2	PROMOCIÓN FAMILIAR	20% de descuento para planes de dos o más miembros de la misma familia.	20	2025-09-01	2025-12-31	ACTIVO	\N
3	SIN PROMOCIÓN	Registro para planes sin promoción aplicada.	0	2000-01-01	2099-12-31	ACTIVO	\N
\.
...
COPY public.recetas (id_res, res_descrip, res_ingrediente, res_calorias, res_carbohidratos, res_proteinas, estado, auditoria) FROM stdin;
1	Ensalada de Pollo	Pollo cocido (150g), lechuga, tomate, zanahoria rallada, aceite de oliva (1 cda), jugo de limón	280	12	35	ACTIVO	\N
2	Omelette de Claras	4 claras de huevo, espinaca, cebolla, pizca de sal y pimienta	130	4	23	ACTIVO	\N
3	Batido Proteico	Leche descremada (200ml), banana (1 unid), proteína en polvo (1 scoop), avena (30g)	310	42	28	ACTIVO	\N
4	Arroz con Verduras	Arroz integral (100g), brócoli, zanahoria, pimiento, salsa de soja	360	65	9	ACTIVO	\N
5	Pechuga al Horno	Pechuga de pollo (200g), ajo, limón, perejil, sal	220	0	45	ACTIVO	\N
\.
...
1	2024-12-28	2024-12-28	Perdida de peso	LUNES A VIERNES	N/Aa	2	3	4	1	1	CONFIRMADO	INSERCION/admin/2024-12-28 20:30:16.644798-03\rMODIFICACION/admin/2025-09-12 21:48:13.61324-04\rMODIFICACION/admin/2025-09-12 21:53:49.789987-04\rMODIFICACION/admin/2025-09-12 21:54:15.482111-04\rCONFIRMACION/admin/2025-09-12 21:55:17.015055-04
2	2025-09-13	2025-09-30	Perdida de peso	LUNES A VIERNES	N/A	\N	1	\N	1	1	CONFIRMADO	INSERCION/admin/2025-09-12 21:55:59.956313-04\rCONFIRMACION/admin/2025-09-12 22:00:00.435613-04
3	2025-09-13	2025-09-30	Perdida de peso	LUNES A MIRCOLES	N/A	\N	2	\N	1	1	PENDIENTE	INSERCION/admin/2025-09-13 07:56:45.711379-04
\.
...
COPY public.serv_alimentaciones_det (id_ali, id_act, cantidad, calorias, carbohidratos, proteinas, auditoria, id_res) FROM stdin;
1	11	50	0.35	80	90	INSERCION DETALLE/admin/2024-12-28 20:30:50.13897-03\rMODIFICACION DE DETALLE/admin/2024-12-28 20:48:45.806782-03\rMODIFICACION DE DETALLE/admin/2024-12-28 20:48:57.819168-03	1
2	11	0.42	45	43	54	INSERCION DETALLE/admin/2025-09-12 21:57:31.255153-04	1
3	10	5	0.10	40	50	INSERCION DETALLE/admin/2025-09-13 07:59:29.984037-04	1
3	11	4343	43	43	34	INSERCION DETALLE/admin/2025-10-13 10:03:47.59584-03	1
3	13	43	43	43	34	INSERCION DETALLE/admin/2025-10-13 10:05:19.812664-03	1
3	12	45	130	4	23	INSERCION DETALLE/admin/2025-10-19 18:00:01.576842-03	2
\.
...
7	2025-09-13 00:01:12	\N	1	3	1	1	ENTRADA	INSERCION/admin/2025-09-12 23:01:13.405335-04
8	\N	2025-09-13 00:01:18	1	3	1	1	SALIDA	INSERCION/admin/2025-09-12 23:01:19.197656-04
9	\N	2025-09-13 00:02:12	3	1	1	1	SALIDA	INSERCION/admin/2025-09-12 23:02:13.183732-04
10	2025-09-13 00:02:17	\N	3	1	1	1	ENTRADA	INSERCION/admin/2025-09-12 23:02:17.680333-04
2	\N	2024-12-28 16:47:29	3	1	1	1	ANULADO	INSERCION/admin/2024-12-28 16:47:29.817323-03\rANULACION/admin/2024-12-28 16:48:17.500847-03\rANULACION/admin/2025-09-12 23:03:14.129309-04
11	2025-09-13 08:44:23	\N	2	4	1	1	ENTRADA	INSERCION/admin/2025-09-13 07:44:23.621088-04
12	\N	2025-09-13 08:45:17	2	4	1	1	SALIDA	INSERCION/admin/2025-09-13 07:45:17.393482-04
13	\N	2025-09-13 08:45:20	2	4	1	1	SALIDA	INSERCION/admin/2025-09-13 07:45:20.570443-04
\.
...
2	2024-12-30	N/A	20	20.76	24.11	3	4	1	1	1	CONFIRMADO	INSERCION/admin/2024-12-30 16:07:05.126224-03\rMODIFICACION/admin/2025-09-12 22:50:32.946108-04\rMODIFICACION/admin/2025-09-12 22:50:38.555771-04\rCONFIRMACION/admin/2025-09-12 22:50:40.153867-04
3	2025-09-13	N/A	20	19.53	22.64	3	4	1	1	1	PENDIENTE	INSERCION/admin/2025-09-13 07:51:34.866599-04
\.
...
3	1	80	INSERCION DETALLE/admin/2025-09-13 07:52:41.101394-04
3	4	1.70	INSERCION DETALLE/admin/2025-09-13 07:53:51.595623-04
\.
...
1	2024-12-28	20	N/A	3	4	1	1	CONFIRMADO	INSERCION/admin/2024-12-28 17:36:09.548163-03\rMODIFICACION/admin/2024-12-28 20:51:28.66092-03\rMODIFICACION/admin/2024-12-28 20:51:33.689886-03\rCONFIRMACION/admin/2024-12-28 20:51:38.479281-03\rMODIFICACION/admin/2025-09-12 22:31:09.869083-04\rMODIFICACION/admin/2025-09-12 22:31:14.461949-04\rMODIFICACION/admin/2025-09-12 22:32:06.585294-04\rMODIFICACION/admin/2025-09-12 22:32:12.153796-04\rCONFIRMACION/admin/2025-09-12 22:34:45.714863-04
2	2025-09-13	27	N/A	1	4	1	1	PENDIENTE	INSERCION/admin/2025-09-13 07:49:30.883876-04
\.
...
2	4	1.80	INSERCION DETALLE/admin/2025-09-13 07:49:42.847403-04
2	1	150	INSERCION DETALLE/admin/2025-09-13 07:50:10.966094-04
1	1	80	INSERCION DETALLE/admin/2025-09-12 19:58:00.343685-04 | TRIGGER UPDATE: 2025-09-13 07:52:41.101394-04
1	4	1.70	INSERCION DETALLE/admin/2025-09-12 22:33:17.325038-04 | TRIGGER UPDATE: 2025-09-13 07:53:51.595623-04
2	2	89	INSERCION DETALLE/admin/2025-10-13 09:14:04.901725-03
\.
...
12	2025-09-20	2025-09-30	N/A	1	1	1	CONFIRMADO	INSERCION/admin/2025-09-20 07:38:41.62846-04\rCONFIRMACION/admin/2025-09-20 07:39:49.659759-04\rCONFIRMACION/admin/2025-09-20 13:50:49.440143-04
13	2025-09-20	2025-09-20	N/A	3	1	1	CONFIRMADO	INSERCION/admin/2025-09-20 13:18:15.221413-04\rCONFIRMACION/admin/2025-09-20 13:18:56.126593-04\rCONFIRMACION/admin/2025-09-20 13:55:51.244286-04\rCONFIRMACION/admin/2025-09-20 13:57:55.566108-04
1	2024-12-28	2025-01-28	N/A	3	1	1	CONFIRMADO	INSERCION/admin/2024-12-28 16:07:38.324533-03\rCONFIRMACION/admin/2024-12-28 16:08:12.286594-03\rCONFIRMACION/admin/2024-12-28 16:24:04.341225-03\rCONFIRMACION/admin/2025-09-12 19:11:28.085585-04
2	2024-12-30	2025-01-30	N/A	3	1	1	CONFIRMADO	INSERCION/admin/2024-12-30 16:03:07.185148-03\rCONFIRMACION/admin/2024-12-30 16:03:41.304182-03\rMODIFICACION/admin/2025-09-12 19:10:14.27225-04\rCONFIRMACION/admin/2025-09-12 19:11:38.651693-04
3	2025-09-12	2025-09-12	N/A	1	1	1	CONFIRMADO	INSERCION/admin/2025-09-12 19:17:51.522421-04\rCONFIRMACION/admin/2025-09-13 07:41:31.230838-04
4	2025-09-13	2025-09-30	N/A	2	1	1	CONFIRMADO	INSERCION/admin/2025-09-13 07:41:55.199741-04\rCONFIRMACION/admin/2025-09-13 07:43:06.836561-04
5	2025-09-13	2025-09-30	N/A	2	1	1	CONFIRMADO	INSERCION/admin/2025-09-13 12:19:49.055781-04\rCONFIRMACION/admin/2025-09-13 13:18:10.413558-04
6	2025-09-13	2025-09-30	N/A	1	1	1	PENDIENTE	INSERCION/admin/2025-09-13 13:18:50.520212-04
7	2025-09-13	2025-09-30	N/A	2	1	1	PENDIENTE	INSERCION/admin/2025-09-13 13:20:01.687737-04
10	2025-09-13	2025-09-13	N/A	3	1	1	CONFIRMADO	INSERCION/admin/2025-09-13 13:23:52.265338-04\rCONFIRMACION/admin/2025-09-13 13:34:32.190722-04
9	2025-09-13	2025-09-13	N/A	1	1	1	CONFIRMADO	INSERCION/admin/2025-09-13 13:22:17.050905-04\rCONFIRMACION/admin/2025-09-13 13:35:26.655711-04
8	2025-09-13	2025-09-13	N/A	2	1	1	ANULADO	INSERCION/admin/2025-09-13 13:21:41.791032-04\rANULACION/admin/2025-09-13 13:35:40.050674-04
14	2025-09-22	2025-09-22	N/A	2	1	1	CONFIRMADO	INSERCION/admin/2025-09-22 19:46:39.652276-04\rCONFIRMACION/admin/2025-09-22 19:49:27.922538-04
11	2025-09-13	2025-09-13	N/A	1	1	1	CONFIRMADO	INSERCION/admin/2025-09-13 13:57:33.869018-04\rCONFIRMACION/admin/2025-09-17 16:35:05.086866-04
15	2025-10-13	2025-10-13	N/A	1	1	1	PENDIENTE	INSERCION/admin/2025-10-12 22:39:28.846745-03
\.
...
1	2	1	70000	INSERCION/admin/2025-09-12 18:57:01.610435-04
2	5	1	250000	INSERCION/admin/2025-09-12 18:57:24.915607-04
3	1	1	200000	INSERCION/admin/2025-09-12 19:24:46.513719-04
3	8	1	50000	INSERCION/admin/2025-09-12 19:25:15.133319-04
4	2	1	70000	INSERCION/admin/2025-09-13 07:42:59.245229-04
11	2	1	70000	INSERCION/admin/2025-09-17 16:31:25.226526-04
12	8	1	50000	INSERCION/admin/2025-09-20 07:38:48.148579-04
12	5	1	250000	INSERCION/admin/2025-09-20 07:38:52.177851-04
13	5	1	250000	INSERCION/admin/2025-09-20 13:18:24.328871-04
13	8	1	50000	INSERCION/admin/2025-09-20 13:18:32.392025-04
14	8	1	50000	INSERCION/admin/2025-09-22 19:48:05.131118-04
\.
...
1	2024-12-28	N/A	3	4	1	1	CONFIRMADO	INSERCION/admin/2024-12-28 20:58:58.009676-03\rMODIFICACION/admin/2025-09-12 22:58:09.339376-04\rMODIFICACION/admin/2025-09-12 22:58:12.778922-04\rCONFIRMACION/admin/2025-09-12 22:58:16.955168-04
2	2025-09-13	n/A	3	4	1	1	CONFIRMADO	INSERCION/admin/2025-09-12 22:58:30.316846-04\rCONFIRMACION/admin/2025-09-12 22:59:06.035225-04
3	2025-09-13	n/a	3	4	1	1	PENDIENTE	INSERCION/admin/2025-09-13 07:59:49.308205-04
\.
...
1	14	N/A	43	INSERCION DETALLE/admin/2025-08-07 18:35:12.414446-04
2	18	N/A	200000	INSERCION DETALLE/admin/2025-09-12 22:58:58.826938-04
3	15	N/A	400000	INSERCION DETALLE/admin/2025-09-13 08:00:37.668662-04
3	16	N/A	200000	INSERCION DETALLE/admin/2025-09-13 08:00:50.798583-04
3	14	N/A	200000	INSERCION DETALLE/admin/2025-10-13 14:35:12.034654-03
\.
...
1	2024-12-28	20	N/A	5	3	4	1	1	CONFIRMADO	INSERCION/admin/2024-12-28 16:50:10.771364-03\rMODIFICACION/admin/2025-09-12 22:14:21.527932-04\rMODIFICACION/admin/2025-09-12 22:14:26.231556-04\rCONFIRMACION/admin/2025-09-12 22:17:11.373236-04
2	2025-09-13	27	n/a	\N	1	4	1	1	CONFIRMADO	INSERCION/admin/2025-09-12 22:17:24.235971-04\rCONFIRMACION/admin/2025-09-13 06:55:26.695325-04
3	2025-09-13	27	N/a	\N	1	4	1	1	PENDIENTE	INSERCION/admin/2025-09-13 07:47:07.974529-04
\.
...
1	8	4	17	50	INSERCION DETALLE/admin/2025-09-12 22:16:38.43088-04	Press banca plano
2	7	4	20	50	INSERCION DETALLE/admin/2025-09-12 22:17:46.344752-04	Press fances
3	5	4	20	60	INSERCION DETALLE/admin/2025-09-13 07:48:47.474737-04	SENTADILLA
3	6	4	20	32	INSERCION DETALLE/admin/2025-10-13 08:52:07.626898-03	Press fances
\.
...
COPY public.servicios_inscripciones_cabecera (id_inscrip, ins_fecha, ins_aprobacion, id_sucursal, id_funcionario, id_cliente, estado, auditoria, ins_edad, ins_peso, ins_altura, ins_seguro_medico) FROM stdin;
1	2024-12-28	2025-09-12	1	1	3	CONFIRMADO	INSERCION/admin/2024-12-28 16:06:13.794257-03\rCONFIRMACION/admin/2024-12-28 16:07:05.246882-03\rMODIFICACION/admin/2025-09-12 19:14:25.005684-04\rCONFIRMACION/admin/2025-09-12 19:16:04.316808-04	18	50	1.60	t
2	2024-12-30	2025-09-12	1	1	1	CONFIRMADO	INSERCION/admin/2024-12-30 16:01:52.378912-03\rMODIFICACION/admin/2024-12-30 16:02:10.084723-03\rMODIFICACION/admin/2025-09-10 21:06:40.196879-04\rMODIFICACION/admin/2025-09-10 21:12:53.897746-04\rMODIFICACION/admin/2025-09-10 21:13:00.757273-04\rMODIFICACION/admin/2025-09-10 21:15:23.118321-04\rMODIFICACION/admin/2025-09-10 21:15:28.438471-04\rMODIFICACION/admin/2025-09-10 21:19:44.512643-04\rMODIFICACION/admin/2025-09-10 21:19:56.654256-04\rMODIFICACION/admin/2025-09-10 21:20:06.168511-04\rMODIFICACION/admin/2025-09-10 21:20:07.00949-04\rMODIFICACION/admin/2025-09-10 21:20:12.27463-04\rMODIFICACION/admin/2025-09-10 21:20:23.960141-04\rMODIFICACION/admin/2025-09-10 21:42:53.372718-04\rMODIFICACION/admin/2025-09-11 13:41:38.00184-04\rCONFIRMACION/admin/2025-09-11 16:40:59.720134-04\rCONFIRMACION/admin/2025-09-12 19:16:25.603045-04	28	70	1.90	f
3	2025-09-13	2025-09-13	1	1	2	CONFIRMADO	INSERCION/admin/2025-09-13 07:38:58.217989-04\rCONFIRMACION/admin/2025-09-13 07:40:37.667028-04	32	80	1.70	t
4	2025-10-12	2025-10-13	1	1	4	PENDIENTE	INSERCION/admin/2025-10-12 22:07:27.21924-03	19	80	1.80	t
\.
...
1	CASA MATRIZ	calle san roque	contacto@energym.com.py	0981000000	xy	/tesis/imagenes/sucursales/1.jpg	ACTIVO	\N	1
2	ÑEMBY	calle san roque	contacto@energym.com.py	0981000000	xy	/tesis/imagenes/sucursales/2.jpg	ACTIVO	\N	1
3	FERNANDO DE LA MORA	calle san roque	contacto@energym.com.py	0981000000	xy	/tesis/imagenes/sucursales/3.jpg	ACTIVO	\N	1
\.
...
2	87654321	FACTURA	2024-02-01	2024-12-31	3	5000	1	2	ACTIVO	\N
3	54321678	FACTURA	2024-03-01	2024-12-31	7	3000	2	1	ACTIVO	\N
7	76543210	FACTURA	2024-05-01	2024-12-31	3	6000	1	3	ACTIVO	\N
1	12345678	FACTURA	2024-01-01	2024-12-31	3	9999	1	1	ACTIVO	\N
9	23456789	FACTURA	2024-06-01	2024-12-31	11	7000	3	1	ACTIVO	\N
5	98765432	NOTA DE DEBITO	2024-01-15	2024-07-31	1	1500	4	1	ACTIVO	\N
6	23456789	FACTURA	2024-04-01	2024-12-31	1	4000	3	2	ACTIVO	\N
4	67890543	NOTA DE CREDITO	2024-01-01	2024-06-30	3	1000	2	2	ACTIVO	\N
8	45678901	NOTA DE REMISION	2024-01-01	2024-03-31	4	500	2	3	ACTIVO	\N
\.


--
-- Data for Name: tipos_duracion; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.tipos_duracion (id_td, td_descrip, td_duracion, estado, auditoria) FROM stdin;
1	MENSUAL	1	ACTIVO	\N
2	TRIMESTRAL	3	ACTIVO	\N
3	ANUAL	1	ACTIVO	\N
4	SEMANAL	1	ACTIVO	\N
5	DIA	1	ACTIVO	\N
\.
...
--
-- Data for Name: tipos_movimientos; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.tipos_movimientos (id_tm, tm_descrip, tm_codigo, tm_afecta_inventario, tm_afecta_contabilidad, estado, auditoria) FROM stdin;
3	VENTA SERVICIO	VTA	f	t	ACTIVO	\N
4	VENTA PRODUCTO	VTA	t	t	ACTIVO	\N
5	DESCUENTO POSTERIOR A LA VENTA	NC	t	t	ACTIVO	\N
6	ERROR EN PRECIO	NC	t	t	ACTIVO	\N
7	ERROR EN CANTIDAD	NC	t	t	ACTIVO	\N
8	ERROR EN PRECIO	ND	t	t	ACTIVO	\N
9	ERROR EN CANTIDAD	ND	t	t	ACTIVO	\N
11	COSTO DE ENVÍO	ND	t	t	ACTIVO	\N
12	INTERESES POR MORA	ND	t	t	ACTIVO	\N
1	ANULACIÓN DE FACTURA	NC	t	t	ACTIVO	\N
2	DEVOLUCIÓN DE PRODUCTOS	NC	t	t	ACTIVO	\N
10	MERCADERÍA DEFECTUOSA O VENCIDA	NC	t	t	ACTIVO	\N
13	REMISIÓN	NR	t	t	ACTIVO	\N
\.
...
COPY public.vehiculos (id_vehiculo, veh_descrip, id_mar, estado, veh_chapa, veh_color) FROM stdin;
1	CAMIÓN	8	ACTIVO	BPG353	BLANCO
2	CAMIÓN	9	ACTIVO	FYX865	BLANCO
\.
...
COPY public.vent_aperturas_cierres (id_vac, id_caja, id_sucursal, id_funcionario, vac_fecha_ape, vac_fecha_cie, vac_monto_efec, vac_monto_cheq, vac_monto_tarj, vac_monto_ape, vac_monto_cie, estado, auditoria, vac_monto_tranf) FROM stdin;
3	1	1	1	2025-09-18 14:05:26	2025-09-20 07:05:44	633500	20000	2800	100000	676300	ABIERTO	INSERCION/admin/2025-09-18 14:08:05.282114-04\rCERRADO/admin/2025-09-20 07:05:47.308239-04\rCERRADO/admin/2025-09-20 07:07:10.442772-04	20000
2	1	1	1	2025-02-18 22:22:05	2025-02-18 22:22:55	140000	0	0	100000	140000	CONFIRMADO	INSERCION/admin/2025-02-18 22:22:54.893655-03\rCERRADO/admin/2025-02-18 22:23:08.481064-03\rCONFIRMACION/admin/2025-02-18 22:23:37.820483-03	0
4	1	1	1	2025-09-20 07:36:23	\N	0	0	0	100000	0	CERRADO	INSERCION/admin/2025-09-20 07:36:48.999188-04	0
1	1	1	1	2025-02-18 15:44:41	2025-02-18 15:48:35	0	100000	65000	110000	225000	CONFIRMADO	INSERCION/admin/2025-02-18 15:44:49.793747-03\rCERRADO/admin/2025-02-18 15:51:36.549409-03\rCONFIRMACION/admin/2025-02-18 21:24:10.007626-03	60000
\.
...
COPY public.vent_arqueo_control (id_varq, arq_fecha, id_vac, id_caja, id_sucursal, id_funcionario, id_fun_solicitante, estado, auditoria, arq_monto_efec, arq_monto_cheq, arq_monto_tarj, arq_monto_tranf) FROM stdin;
1	2025-10-13 19:39:46.667537	3	1	1	1	2	PENDIENTE	INSERCION/admin/2025-10-13 19:39:46.667537-03	20000	20000	20000	20000
\.


--
-- Data for Name: vent_cobros_cab; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.vent_cobros_cab (id_cob, id_vac, id_caja, id_sucursal, id_funcionario, cob_fecha, estado, auditoria, id_vc) FROM stdin;
1	1	1	1	1	2025-04-15 00:20:34	CONFIRMADO	INSERCION/admin/2025-04-14 20:20:49.752861-04\rANULACION/admin/2025-09-04 19:36:15.46261-04\rCONFIRMACION/admin/2025-09-04 19:46:28.85128-04	1
2	1	1	1	1	2025-08-26 01:07:46	CONFIRMADO	INSERCION/admin/2025-08-25 21:07:56.265123-04\rCONFIRMACION/admin/2025-09-04 19:46:50.433093-04	9
3	1	1	1	1	2025-09-04 23:48:05	ANULADO	INSERCION/admin/2025-09-04 19:48:15.813368-04\rANULACION/admin/2025-09-14 18:33:18.04214-04	4
4	1	1	1	1	2025-09-14 22:42:17	CONFIRMADO	INSERCION/admin/2025-09-14 18:42:28.697715-04\rCONFIRMACION/admin/2025-09-19 22:20:52.787071-04\rCONFIRMACION/admin/2025-09-20 07:52:58.090999-04	10
5	3	1	1	1	2025-09-20 15:50:40	CONFIRMADO	INSERCION/admin/2025-09-20 11:51:17.930695-04\rCONFIRMACION/admin/2025-09-22 19:54:29.310601-04	14
6	3	1	1	1	2025-09-22 23:54:30	CONFIRMADO	INSERCION/admin/2025-09-22 19:54:45.266182-04\rCONFIRMACION/admin/2025-09-22 19:58:47.313349-04	16
7	3	1	1	1	2025-10-02 23:01:09	CONFIRMADO	INSERCION/admin/2025-10-02 19:01:18.099908-04\rCONFIRMACION/admin/2025-10-13 23:18:56.978806-03	16
8	3	1	1	1	2025-10-14 02:19:01	CONFIRMADO	INSERCION/admin/2025-10-13 23:19:05.85927-03\rANULACION/admin/2025-10-16 22:31:16.772335-03\rCONFIRMACION/admin/2025-10-16 22:40:10.716115-03\rCONFIRMACION/admin/2025-10-17 18:46:12.253593-03	9
\.


--
-- Data for Name: vent_cobros_det; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.vent_cobros_det (id_cob, id_cue, id_fc, cob_monto_efe, auditoria, monto) FROM stdin;
2	1	4	76750	INSERCION/admin/2025-09-02 21:12:19.522317-04	\N
2	2	2	76750	INSERCION/admin/2025-09-02 21:14:41.27273-04	\N
2	3	2	76750	INSERCION/admin/2025-09-02 21:18:27.797675-04	\N
7	28	4	480000	INSERCION/admin/2025-10-02 19:52:22.067524-04	\N
1	5	1	21417	INSERCION/admin/2025-09-03 21:51:59.379155-04	\N
1	14	1	21417	INSERCION/admin/2025-09-03 21:53:09.490436-04	\N
1	15	1	21417	INSERCION/admin/2025-09-03 22:03:53.935552-04	\N
1	16	3	21417	INSERCION/admin/2025-09-03 22:09:25.503499-04	\N
4	20	3	0	INSERCION/admin/2025-09-14 19:57:40.29208-04	\N
5	27	1	0	INSERCION/admin/2025-09-20 11:51:28.837288-04	\N
6	28	2	0	INSERCION/admin/2025-09-22 19:55:00.859623-04	\N
8	1	4	76750	INSERCION/admin/2025-10-16 21:56:56.127435-03	0
8	4	4	76700	INSERCION/admin/2025-10-15 23:19:07.458299-03	76750
\.
...
COPY public.vent_cuentas_cobrar (id_cue, id_vc, cue_monto, cue_saldo, estado, auditoria, nro_cuota, fecha_intervalo) FROM stdin;
27	14	264000	264000	PENDIENTE	INSERCION/admin/2025-09-20 07:41:35.623212-04	1	2025-09-20
1	9	76750	0	PAGADO	INSERCION/admin/2025-08-25 20:49:37.62174-04	1	2025-05-27
17	2	416667	416667	PENDIENTE	INSERCION/admin/2025-08-28 20:58:29.551761-04	1	2025-03-27
18	2	416667	416667	PENDIENTE	INSERCION/admin/2025-08-28 20:58:29.551761-04	2	2025-04-26
19	2	416667	416667	PENDIENTE	INSERCION/admin/2025-08-28 20:58:29.551761-04	3	2025-05-26
20	10	-137500	-137500	PENDIENTE	INSERCION/admin/2025-09-14 18:41:54.229148-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04	1	2025-10-14
6	1	21417	21417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	2	2025-09-27
7	1	21417	21417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	3	2025-10-27
8	1	21417	21417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	4	2025-11-27
9	1	21417	21417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	5	2025-12-27
10	1	21417	21417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	6	2026-01-27
11	1	21417	21417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	7	2026-02-27
12	1	21417	21417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	8	2026-03-27
13	1	21417	21417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	9	2026-04-27
21	10	-137500	-137500	PENDIENTE	INSERCION/admin/2025-09-14 18:41:54.229148-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04	2	2025-11-13
22	10	-137500	-137500	PENDIENTE	INSERCION/admin/2025-09-14 18:41:54.229148-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04	3	2025-12-13
23	10	-137500	-137500	PENDIENTE	INSERCION/admin/2025-09-14 18:41:54.229148-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04\rAJUSTADO/admin/2025-09-25 13:59:04.864788-04	4	2026-01-12
28	16	480000	0	PAGADO	INSERCION/admin/2025-09-22 19:54:16.748164-04	1	2025-09-22
4	9	76750	0	PAGADO	INSERCION/admin/2025-08-25 20:49:37.62174-04	4	2025-08-27
5	1	21417	410	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	1	2025-08-27
14	1	21417	11417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	10	2026-05-27
15	1	21417	231417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	11	2026-06-27
16	1	21417	21417	PENDIENTE	INSERCION/admin/2025-08-25 21:00:28.281185-04	12	2026-07-27
2	9	76750	0	PAGADO	INSERCION/admin/2025-08-25 20:49:37.62174-04	2	2025-06-27
3	9	76750	0	PAGADO	INSERCION/admin/2025-08-25 20:49:37.62174-04	3	2025-07-27
24	11	140000	140000	PENDIENTE	INSERCION/admin/2025-09-17 22:42:42.402827-04	1	2025-09-18
25	12	250000	250000	PENDIENTE	INSERCION/admin/2025-09-17 23:15:14.450199-04	1	2025-09-18
26	13	20000	20000	PENDIENTE	INSERCION/admin/2025-09-20 07:30:12.719425-04	1	2025-09-18
\.
...
COPY public.vent_nota_cab (id_not, not_fecha_elab, not_fecha_emis, not_tipo_nota, not_observacion, id_vc, id_sucursal, id_funcionario, id_cliente, estado, auditoria, id_tm, id_tim, not_nro_documento, id_vehiculo, id_chofer) FROM stdin;
1	2025-09-25 22:05:30.535164	2025-02-25 00:00:00	CREDITO	n/a	3	1	1	2	PENDIENTE	INSERCION/admin/2025-09-25 22:05:30.535164-04	5	4	002-002-0000002	\N	\N
2	2025-09-27 07:50:43.241142	2025-02-25 00:00:00	CREDITO	n/a	3	1	1	2	CONFIRMADO	INSERCION/admin/2025-09-27 07:50:43.241142-04\rCONFIRMACION/admin/2025-09-27 07:55:56.531502-04	1	4	002-002-0000002	\N	\N
3	2025-09-27 07:56:30.314477	2025-02-26 00:00:00	DEBITO	n/a	5	1	1	3	PENDIENTE	INSERCION/admin/2025-09-27 07:56:30.314477-04	11	5	004-001-0000001	\N	\N
4	2025-10-01 00:00:00	2025-02-25 00:00:00	REMISION	N/A	3	1	1	2	CONFIRMADO	INSERCION/admin/2025-10-01 15:22:51.509742-04\rMODIFICACION/admin/2025-10-01 21:28:57.387114-04\rCONFIRMACION/admin/2025-10-01 21:29:55.127955-04	\N	8	002-003-0000003	1	11
5	2025-10-01 21:30:14.902448	2025-02-25 00:00:00	REMISION	N/A	3	1	1	2	PENDIENTE	INSERCION/admin/2025-10-01 21:30:14.902448-04	\N	8	002-003-0000004	1	11
\.
...
COPY public.vent_recaudacion_depositar (id_vrd, id_vac, id_caja, id_sucursal, id_funcionario, vrd_fecha, vrd_monto_cie, estado, auditoria) FROM stdin;
1	3	1	1	1	2025-09-20	0	CONFIRMADO	INSERCION/admin/2025-09-20 07:05:47.308239-04
2	3	1	1	1	2025-09-20	0	CONFIRMADO	INSERCION/admin/2025-09-20 07:07:10.442772-04
\.
...
COPY public.ventas_cab (id_vc, vc_fecha, vc_tipo_factura, vc_cuota, vc_intervalo, vc_nro_factura, id_tim, id_sucursal, id_funcionario, id_cliente, estado, auditoria, id_tm) FROM stdin;
12	2025-09-18	CONTADO	1	0	003-001-0000007	9	1	1	2	CONFIRMADO	INSERCION/admin/2025-09-17 23:07:42.467396-04\rCONFIRMACION/admin/2025-09-17 23:11:50.376815-04\rCONFIRMACION/admin/2025-09-17 23:15:14.450199-04	4
4	2025-02-25	CREDITO	12	12	003-001-0000008	2	1	1	1	CONFIRMADO	INSERCION/admin/2025-02-24 21:54:34.664872-03\rCONFIRMACION/admin/2025-02-24 22:22:49.449897-03\rCONFIRMACION/admin/2025-02-24 22:30:37.333658-03	4
5	2025-02-26	CONTADO	0	0	003-001-0000009	3	1	1	3	CONFIRMADO	INSERCION/admin/2025-02-26 15:04:46.989909-03\rCONFIRMACION/admin/2025-02-26 15:06:42.457981-03	4
13	2025-09-18	CONTADO	1	0	003-001-0000010	9	1	1	3	CONFIRMADO	INSERCION/admin/2025-09-17 23:18:53.166558-04\rCONFIRMACION/admin/2025-09-20 07:30:12.719425-04	4
6	2025-02-26	CONTADO	0	0	003-001-0000012	3	1	1	2	CONFIRMADO	INSERCION/admin/2025-02-26 15:37:00.089113-03\rCONFIRMACION/admin/2025-02-26 15:37:50.67759-03	4
7	2025-02-26	CONTADO	0	0	003-001-0000013	3	1	1	2	CONFIRMADO	INSERCION/admin/2025-02-26 15:38:32.443715-03\rCONFIRMACION/admin/2025-02-26 15:40:32.762475-03	4
8	2025-02-26	CONTADO	0	0	003-001-0000014	3	1	1	3	CONFIRMADO	INSERCION/admin/2025-02-26 15:59:44.672865-03\rCONFIRMACION/admin/2025-07-18 10:09:30.797183-04	4
14	2025-09-20	CONTADO	1	0	003-001-0000015	9	1	1	1	CONFIRMADO	INSERCION/admin/2025-09-20 07:40:57.207397-04\rCONFIRMACION/admin/2025-09-20 07:41:35.623212-04	4
15	2025-09-22	CREDITO	3	133	003-001-0000016	9	1	1	2	ANULADO	INSERCION/admin/2025-09-22 19:52:13.15516-04\rANULACION/admin/2025-09-22 19:52:35.555605-04	4
11	2025-09-18	CONTADO	1	0	003-001-0000001	9	1	1	1	CONFIRMADO	INSERCION/admin/2025-09-17 20:46:25.019303-04\rCONFIRMACION/admin/2025-09-17 21:32:09.624054-04\rCONFIRMACION/admin/2025-09-17 22:20:37.262693-04\rCONFIRMACION/admin/2025-09-17 22:28:04.514219-04\rCONFIRMACION/admin/2025-09-17 22:42:42.402827-04	3
1	2025-01-28	CREDITO	12	15	003-001-0000002	8	1	1	3	CONFIRMADO	INSERCION/admin/2025-01-28 14:51:47.184299-03\rMODIFICACION/admin/2025-02-02 14:39:11.637967-03\rMODIFICACION/admin/2025-02-02 14:47:28.399351-03\rMODIFICACION/admin/2025-02-02 14:47:35.158329-03\rCONFIRMACION/admin/2025-02-02 20:43:05.621405-03\rCONFIRMACION/admin/2025-02-24 21:02:08.061068-03\rCONFIRMACION/admin/2025-08-25 21:00:28.281185-04	4
9	2025-08-26	CREDITO	4	15	003-001-0000003	7	1	1	1	CONFIRMADO	INSERCION/admin/2025-08-25 20:41:08.292873-04\rCONFIRMACION/admin/2025-08-25 20:42:28.368659-04\rCONFIRMACION/admin/2025-08-25 20:49:37.62174-04	4
2	2025-02-25	CREDITO	3	30	003-001-0000004	1	1	1	2	CONFIRMADO	INSERCION/admin/2025-02-24 21:13:41.566348-03\rCONFIRMACION/admin/2025-08-28 20:58:29.551761-04	4
10	2025-09-14	CREDITO	4	30	003-001-0000005	9	1	1	1	CONFIRMADO	INSERCION/admin/2025-09-14 18:41:18.258836-04\rCONFIRMACION/admin/2025-09-14 18:41:54.229148-04	4
3	2025-02-25	CREDITO	12	15	003-001-0000006	1	1	1	2	CONFIRMADO	INSERCION/admin/2025-02-24 21:27:02.305812-03\rCONFIRMACION/admin/2025-02-24 21:46:59.181475-03	4
16	2025-09-22	CONTADO	1	0	003-001-0000017	9	1	1	2	CONFIRMADO	INSERCION/admin/2025-09-22 19:52:55.615335-04\rCONFIRMACION/admin/2025-09-22 19:54:16.748164-04	3
17	2025-09-27	CONTADO	1	0	003-002-0000001	6	1	1	1	PENDIENTE	INSERCION/admin/2025-09-27 13:54:26.935043-04	4
\.
...
COPY public.ventas_det (id_vc, id_item, cantidad, precio, auditoria, id_sucursal) FROM stdin;
1	5	1	7000	\N	1
2	1	5	250000	\N	1
3	2	7	250000	\N	1
4	5	12	7000	\N	1
...
3	2023-10-27	2023-10-27		1	1	1	CERRADO	INSERCION/admin/2023-10-27 21:41:26.74757-03\rCONFIRMACION/usubeta/2023-11-08 21:04:29.939857-03\rCERRAR/admin/2025-02-24 21:02:08.061068-03
4	2023-11-08	2023-11-08		1	3	1	CERRADO	INSERCION/usubeta/2023-11-08 21:04:35.504479-03\rCONFIRMACION/admin/2025-02-24 21:46:22.834153-03\rCERRAR/admin/2025-02-24 21:46:59.181475-03
5	2025-08-29	2025-08-30	dasd	1	1	3	CERRADO	INSERCION/admin/2025-08-29 21:06:37.816621-04\rCONFIRMACION/admin/2025-09-20 06:45:28.872336-04\rCERRAR/admin/2025-09-20 07:30:12.719425-04
6	2025-09-20	2025-09-20	n	1	1	3	CERRADO	INSERCION/admin/2025-09-20 07:35:06.228768-04\rCONFIRMACION/admin/2025-09-20 07:36:02.309479-04\rCERRAR/admin/2025-09-20 07:41:35.623212-04
7	2025-10-13	2025-10-13	n/a	1	1	3	PENDIENTE	INSERCION/admin/2025-10-13 17:58:04.231794-03
\.
...
--
-- Data for Name: ventas_servicio_det; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.ventas_servicio_det (id_vc, id_cm, nro_cuota, precio, auditoria) FROM stdin;
11	1	1	140000	\N
16	4	1	480000	\N
\.
...
--
-- Name: clientes_estados_salud clientes_estados_salud_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.clientes_estados_salud
    ADD CONSTRAINT clientes_estados_salud_pkey PRIMARY KEY (id_cliente, id_es);
...
--
-- Name: cobros_cheques cobros_cheques_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cobros_cheques
    ADD CONSTRAINT cobros_cheques_pkey PRIMARY KEY (id_che);


--
-- Name: cobros_tarjetas cobros_tarjetas_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cobros_tarjetas
    ADD CONSTRAINT cobros_tarjetas_pkey PRIMARY KEY (id_ctar);


--
-- Name: cobros_transferencia cobros_transferencia_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cobros_transferencia
    ADD CONSTRAINT cobros_transferencia_pkey PRIMARY KEY (id_ctra);
...
--
-- Name: cuotas_membresias cuotas_membresias_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cuotas_membresias
    ADD CONSTRAINT cuotas_membresias_pkey PRIMARY KEY (id_cm);
...
ALTER TABLE ONLY public.devoluciones
    ADD CONSTRAINT devoluciones_pkey PRIMARY KEY (id_cob, id_cue);
...
--
-- Name: entidades_adheridas entidades_adheridas_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.entidades_adheridas
    ADD CONSTRAINT entidades_adheridas_pkey PRIMARY KEY (id_ee, id_mt);
...
--
-- Name: estados_salud estados_salud_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.estados_salud
    ADD CONSTRAINT estados_salud_pkey PRIMARY KEY (id_es);


--
-- Name: formas_cobros formas_cobros_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.formas_cobros
    ADD CONSTRAINT formas_cobros_pkey PRIMARY KEY (id_fc);
...
--
-- Name: marcas_tarjetas marcas_tarjetas_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.marcas_tarjetas
    ADD CONSTRAINT marcas_tarjetas_pkey PRIMARY KEY (id_mt);
...
ALTER TABLE ONLY public.recetas
    ADD CONSTRAINT recetas_pkey PRIMARY KEY (id_res);
...
ALTER TABLE ONLY public.stock
    ADD CONSTRAINT stock_pk PRIMARY KEY (id_sucursal, id_item);
...
--
-- Name: tipos_duracion tipos_duracion_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.tipos_duracion
    ADD CONSTRAINT tipos_duracion_pkey PRIMARY KEY (id_td);
...
--
-- Name: tipos_movimientos tipos_movimientos_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.tipos_movimientos
    ADD CONSTRAINT tipos_movimientos_pkey PRIMARY KEY (id_tm);
...
--
-- Name: vent_cobros_cab vent_cobros_cab_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_cobros_cab
    ADD CONSTRAINT vent_cobros_cab_pkey PRIMARY KEY (id_cob);


--
-- Name: vent_cobros_det vent_cobros_det_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_cobros_det
    ADD CONSTRAINT vent_cobros_det_pkey PRIMARY KEY (id_cob, id_cue);
...
--
-- Name: ventas_servicio_det ventas_servicio_det_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.ventas_servicio_det
    ADD CONSTRAINT ventas_servicio_det_pkey PRIMARY KEY (id_vc, id_cm);


--
-- Name: compras_presupuestos_detalle tr_actualizar_precio_compra; Type: TRIGGER; Schema: public; Owner: postgres
--

CREATE TRIGGER tr_actualizar_precio_compra AFTER INSERT ON public.compras_presupuestos_detalle FOR EACH ROW EXECUTE PROCEDURE public.sp_actualizar_precio_compra();


--
-- Name: compras_presupuestos_pedidos tr_actualizar_precio_compra_pedidos; Type: TRIGGER; Schema: public; Owner: postgres
--

CREATE TRIGGER tr_actualizar_precio_compra_pedidos AFTER INSERT ON public.compras_presupuestos_pedidos FOR EACH ROW EXECUTE PROCEDURE public.sp_actualizar_precio_compra();
...
--
-- Name: clientes_estados_salud clientes_estados_salud_id_cliente_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.clientes_estados_salud
    ADD CONSTRAINT clientes_estados_salud_id_cliente_fkey FOREIGN KEY (id_cliente) REFERENCES public.clientes(id_cliente);


--
-- Name: clientes_estados_salud clientes_estados_salud_id_es_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.clientes_estados_salud
    ADD CONSTRAINT clientes_estados_salud_id_es_fkey FOREIGN KEY (id_es) REFERENCES public.estados_salud(id_es);
...
--
-- Name: cobros_cheques cobros_cheques_id_cob_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cobros_cheques
    ADD CONSTRAINT cobros_cheques_id_cob_fkey FOREIGN KEY (id_cob) REFERENCES public.vent_cobros_cab(id_cob);


--
-- Name: cobros_cheques cobros_cheques_id_ee_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cobros_cheques
    ADD CONSTRAINT cobros_cheques_id_ee_fkey FOREIGN KEY (id_ee) REFERENCES public.entidades_emisoras(id_ee);


--
-- Name: cobros_tarjetas cobros_tarjetas_id_cob_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cobros_tarjetas
    ADD CONSTRAINT cobros_tarjetas_id_cob_fkey FOREIGN KEY (id_cob) REFERENCES public.vent_cobros_cab(id_cob);


--
-- Name: cobros_tarjetas cobros_tarjetas_id_ee_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cobros_tarjetas
    ADD CONSTRAINT cobros_tarjetas_id_ee_fkey FOREIGN KEY (id_ee, id_mt) REFERENCES public.entidades_adheridas(id_ee, id_mt);


--
-- Name: cobros_transferencia cobros_transferencia_id_cob_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cobros_transferencia
    ADD CONSTRAINT cobros_transferencia_id_cob_fkey FOREIGN KEY (id_cob) REFERENCES public.vent_cobros_cab(id_cob);


--
-- Name: cobros_transferencia cobros_transferencia_id_ee_des_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cobros_transferencia
    ADD CONSTRAINT cobros_transferencia_id_ee_des_fkey FOREIGN KEY (id_ee_des) REFERENCES public.entidades_emisoras(id_ee);


--
-- Name: cobros_transferencia cobros_transferencia_id_ee_ori_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cobros_transferencia
    ADD CONSTRAINT cobros_transferencia_id_ee_ori_fkey FOREIGN KEY (id_ee_ori) REFERENCES public.entidades_emisoras(id_ee);
...
ALTER TABLE ONLY public.comp_ajustes_det
    ADD CONSTRAINT comp_ajustes_det_id_item_fkey FOREIGN KEY (id_sucursal, id_item) REFERENCES public.stock(id_sucursal, id_item);
...
--
-- Name: comp_nota_cab comp_nota_cab_id_tm_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.comp_nota_cab
    ADD CONSTRAINT comp_nota_cab_id_tm_fkey FOREIGN KEY (id_tm) REFERENCES public.tipos_movimientos(id_tm);
...
ALTER TABLE ONLY public.comp_transfers_det
    ADD CONSTRAINT comp_transfers_det_id_item_fkey FOREIGN KEY (id_sucursal, id_item) REFERENCES public.stock(id_sucursal, id_item);
...
ALTER TABLE ONLY public.compras_detalle
    ADD CONSTRAINT compras_detalle_id_item_fkey FOREIGN KEY (id_sucursal, id_item) REFERENCES public.stock(id_sucursal, id_item);
...
--
-- Name: cuotas_membresias cuotas_membresias_id_cliente_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cuotas_membresias
    ADD CONSTRAINT cuotas_membresias_id_cliente_fkey FOREIGN KEY (id_cliente) REFERENCES public.clientes(id_cliente);


--
-- Name: cuotas_membresias cuotas_membresias_id_mem_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.cuotas_membresias
    ADD CONSTRAINT cuotas_membresias_id_mem_fkey FOREIGN KEY (id_mem) REFERENCES public.serv_membresias_cab(id_mem);
...
--
-- Name: devoluciones devoluciones_id_cob_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.devoluciones
    ADD CONSTRAINT devoluciones_id_cob_fkey FOREIGN KEY (id_cob, id_cue) REFERENCES public.vent_cobros_det(id_cob, id_cue);


--
-- Name: devoluciones devoluciones_id_fc_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.devoluciones
    ADD CONSTRAINT devoluciones_id_fc_fkey FOREIGN KEY (id_fc) REFERENCES public.formas_cobros(id_fc);


--
-- Name: entidades_adheridas entidades_adheridas_id_ee_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.entidades_adheridas
    ADD CONSTRAINT entidades_adheridas_id_ee_fkey FOREIGN KEY (id_ee) REFERENCES public.entidades_emisoras(id_ee);


--
-- Name: entidades_adheridas entidades_adheridas_id_mt_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.entidades_adheridas
    ADD CONSTRAINT entidades_adheridas_id_mt_fkey FOREIGN KEY (id_mt) REFERENCES public.marcas_tarjetas(id_mt);
...
--
-- Name: planes_servicios planes_servicios_id_promo_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.planes_servicios
    ADD CONSTRAINT planes_servicios_id_promo_fkey FOREIGN KEY (id_promo) REFERENCES public.promociones(id_promo);


--
-- Name: planes_servicios planes_servicios_id_td_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.planes_servicios
    ADD CONSTRAINT planes_servicios_id_td_fkey FOREIGN KEY (id_td) REFERENCES public.tipos_duracion(id_td);
...
--
-- Name: serv_alimentaciones_det serv_alimentaciones_det_id_res_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.serv_alimentaciones_det
    ADD CONSTRAINT serv_alimentaciones_det_id_res_fkey FOREIGN KEY (id_res) REFERENCES public.recetas(id_res);
...
--
-- Name: stock stock_item_fk; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.stock
    ADD CONSTRAINT stock_item_fk FOREIGN KEY (id_item) REFERENCES public.items(id_item);


--
-- Name: stock stock_sucursal_fk; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.stock
    ADD CONSTRAINT stock_sucursal_fk FOREIGN KEY (id_sucursal) REFERENCES public.deposito(id_sucursal);
...
--
-- Name: vent_cobros_cab vent_cobros_cab_id_vac_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_cobros_cab
    ADD CONSTRAINT vent_cobros_cab_id_vac_fkey FOREIGN KEY (id_vac, id_caja, id_sucursal, id_funcionario) REFERENCES public.vent_aperturas_cierres(id_vac, id_caja, id_sucursal, id_funcionario);


--
-- Name: vent_cobros_cab vent_cobros_cab_id_vc_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_cobros_cab
    ADD CONSTRAINT vent_cobros_cab_id_vc_fkey FOREIGN KEY (id_vc) REFERENCES public.ventas_cab(id_vc);


--
-- Name: vent_cobros_det vent_cobros_det_id_cob_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_cobros_det
    ADD CONSTRAINT vent_cobros_det_id_cob_fkey FOREIGN KEY (id_cob) REFERENCES public.vent_cobros_cab(id_cob);


--
-- Name: vent_cobros_det vent_cobros_det_id_cue_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_cobros_det
    ADD CONSTRAINT vent_cobros_det_id_cue_fkey FOREIGN KEY (id_cue) REFERENCES public.vent_cuentas_cobrar(id_cue);


--
-- Name: vent_cobros_det vent_cobros_det_id_fc_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_cobros_det
    ADD CONSTRAINT vent_cobros_det_id_fc_fkey FOREIGN KEY (id_fc) REFERENCES public.formas_cobros(id_fc);
...
--
-- Name: vent_nota_cab vent_nota_cab_id_chofer_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_nota_cab
    ADD CONSTRAINT vent_nota_cab_id_chofer_fkey FOREIGN KEY (id_chofer) REFERENCES public.funcionarios(id_funcionario);
...
--
-- Name: vent_nota_cab vent_nota_cab_id_tim_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_nota_cab
    ADD CONSTRAINT vent_nota_cab_id_tim_fkey FOREIGN KEY (id_tim) REFERENCES public.timbrados(id_tim);


--
-- Name: vent_nota_cab vent_nota_cab_id_tm_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_nota_cab
    ADD CONSTRAINT vent_nota_cab_id_tm_fkey FOREIGN KEY (id_tm) REFERENCES public.tipos_movimientos(id_tm);
...
--
-- Name: vent_nota_cab vent_nota_cab_id_vehiculo_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.vent_nota_cab
    ADD CONSTRAINT vent_nota_cab_id_vehiculo_fkey FOREIGN KEY (id_vehiculo) REFERENCES public.vehiculos(id_vehiculo);
...
--
-- Name: ventas_cab ventas_cab_id_tm_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.ventas_cab
    ADD CONSTRAINT ventas_cab_id_tm_fkey FOREIGN KEY (id_tm) REFERENCES public.tipos_movimientos(id_tm);
...
ALTER TABLE ONLY public.ventas_det
    ADD CONSTRAINT ventas_det_id_item_fkey FOREIGN KEY (id_sucursal, id_item) REFERENCES public.stock(id_sucursal, id_item);
...
--
-- Name: ventas_servicio_det ventas_servicio_det_id_cm_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.ventas_servicio_det
    ADD CONSTRAINT ventas_servicio_det_id_cm_fkey FOREIGN KEY (id_cm) REFERENCES public.cuotas_membresias(id_cm);


--
-- Name: ventas_servicio_det ventas_servicio_det_id_vc_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.ventas_servicio_det
    ADD CONSTRAINT ventas_servicio_det_id_vc_fkey FOREIGN KEY (id_vc) REFERENCES public.ventas_cab(id_vc);
