...
BEGIN
    IF operacion = 1 THEN 
        ...
        PERFORM 1 FROM serv_asistencias_cab WHERE id_cliente = xid_cliente
		  --AND estado = 'ENTRADA' -- Que todavía tenga una entrada abierta
		  AND asi_entrada >= NOW() - INTERVAL '10 MINUTE'; -- La entrada fue hace menos de 10 minutos (asumiendo que xasi_entrada es NOW() o muy cercano)
		IF FOUND THEN 
		    RAISE EXCEPTION 'YA TIENES UNA ENTRADA ACTIVA Y FUE REGISTRADA RECIENTEMENTE';
		END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM servicios_inscripciones_cabecera WHERE id_cliente = xid_cliente) THEN
        ...
        END IF;
      	...
      	PERFORM 1 FROM serv_asistencias_cab WHERE id_cliente = xid_cliente
		  --AND estado = 'ENTRADA' -- Que todavía tenga una entrada abierta
		  AND asi_salida >= NOW() - INTERVAL '10 MINUTE'; -- La entrada fue hace menos de 10 minutos (asumiendo que xasi_entrada es NOW() o muy cercano)
		IF FOUND THEN 
		    RAISE EXCEPTION 'YA TIENES UNA SALIDA ACTIVA Y FUE REGISTRADA RECIENTEMENTE';
		END IF;
	
		PERFORM 1 FROM serv_asistencias_cab WHERE id_cliente = xid_cliente AND estado = 'ENTRADA'; -- Que todavía tenga una entrada abierta
		if NOT FOUND then
		    RAISE EXCEPTION 'NO TIENE UNA ENTRADA ACTIVA REGISTRADA. DEBE MARCAR SU ENTRADA PRIMERO';
		END IF;
      
        -- Registrar salida
        UPDATE serv_asistencias_cab
        SET id_cliente = xid_cliente,
        	asi_salida = Xasi_salida,
            --id_mem = xid_mem,
        	estado = 'SALIDA',
            auditoria = COALESCE(auditoria, '') || CHR(13) || 'SALIDA/' || usuario || '/' || NOW()
        WHERE id_cliente = xid_cliente and estado = 'ENTRADA';
        RAISE NOTICE 'DATOS MODIFICADOS CON ÉXITO';
...
BEGIN 
    IF operacion = 1 THEN -- Insertar compra_pedido_cabecera
		...
		VALUES ((SELECT COALESCE(MAX(id_not), 0) + 1 FROM comp_nota_cab), 
	     ...
	     FOR d IN select distinct on(id_item) * FROM v_compras_consolidacion WHERE id_cc = xid_cc LOOP
    ...
    ELSIF operacion = 3 THEN 
     ...
     PERFORM * FROM compras_cabecera WHERE id_cc = xid_cc AND estado = 'PENDIENTE'; 
       ...
       IF NOT FOUND THEN
            RAISE EXCEPTION 'ESTE MOVIMIENTO NO POSEE DETALLES CARGADOS';
        END IF;
       v_id_sucursal := xid_sucursal;
    -- ACTUALIZAR STOCK CON UN FOR
    FOR d IN SELECT id_item, catidad FROM v_compras_consolidacion  WHERE id_cc = xid_cc loop 
           IF EXISTS (
	        SELECT 1
	        FROM stock
	        WHERE id_sucursal = v_id_sucursal AND id_item = d.id_item
	    ) THEN
	        -- Actualizar stock sumando la cantidad
	        UPDATE stock
	    ...
	    ELSE
	        ...
	        INSERT INTO stock (id_sucursal, id_item, stock_cantidad, estado)
	        VALUES (v_id_sucursal, d.id_item, d.catidad, 'ACTIVO');
	    END IF;
...
BEGIN 
   ...
   PERFORM 1 FROM serv_mediciones_cab WHERE id_med = xid_med and id_cliente = xid_cliente; 
        IF not FOUND THEN
            RAISE EXCEPTION 'NO EXITE LA MEDICIÓN DEL CLIENTE';
    END IF;
		...
		FOR d IN SELECT id_act, valor FROM serv_mediciones_det WHERE id_med = xid_med LOOP
		    IF d.id_act = 4 THEN
		        INSERT INTO serv_evoluciones_det (id_evo, id_act, valor, auditoria)
		        VALUES (
		            (SELECT MAX(id_evo) FROM serv_evoluciones_cab),
		            d.id_act,
		            d.valor,
		            'INSERCION DETALLE/' || usuario || '/' || NOW()
		        );
...
--
-- Name: sp_vent_cobros(integer, timestamp without time zone, integer, integer, integer, integer, integer, integer, integer, numeric, numeric, character varying, date, numeric, character varying, integer, integer, character varying, date, character varying, numeric, integer, character varying, numeric, character varying, character varying, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.sp_vent_cobros(xid_cob integer, xcob_fecha timestamp without time zone, xid_vac integer, xid_caja integer, xid_vc integer, xid_sucursal integer, xid_funcionario integer, xid_cue integer, xid_fc integer, xcob_monto_efe numeric, xmonto numeric, xche_nro_cheque character varying, xche_vencimiento date, xche_monto numeric, xche_tipo_cheque character varying, xid_ee integer, xid_ee_des integer, xtar_nro_tarjeta character varying, xtar_vencimiento date, xtar_autorizacion character varying, xtar_monto numeric, xid_mt integer, xtra_nro_cuenta character varying, xtra_monto numeric, xtra_motivo character varying, usuario character varying, operacion integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    d RECORD;
...
BEGIN 
    ...
    ELSIF operacion = 6 THEN -- Modificar compra_pedido_detalle
		...
		--perform * from vent_cuentas_cobrar where id_cue = xid_cue and cue_saldo > monto_cuota;
	      --if not found then
	      	--raise exception 'EL PAGO EXCEDE EL MONTO DE LA CUOTA';
	     --end if;
       IF xcob_monto_efe <= 0 and xid_fc = 4 THEN
		    RAISE EXCEPTION 'EL MONTO DEL COBRO DEBE SER MAYOR QUE CERO';
		END IF;
    ...
    IF xtar_monto <= 0 THEN
     ...
     insert into cobros_tarjetas (id_ctar,tar_nro_tarjeta,tar_autorizacion,tar_vencimiento,tar_monto,id_cob,id_ee,id_mt,estado,auditoria)
     values((SELECT COALESCE(MAX(id_ctar), 0) + 1 FROM cobros_tarjetas),
    xtar_nro_tarjeta,xtar_autorizacion,xtar_vencimiento,xtar_monto,xid_cob,xid_ee,xid_mt,'ACTIVO','INSERCION/' || usuario || '/' || NOW());
    		RAISE NOTICE 'DATOS AÑADIDO CON ÉXITO';
       ...
       select id_tm into c from v_ventas_cab where id_vc = xid_vc;
		...
		VALUES ((SELECT COALESCE(MAX(id_not), 0) + 1 FROM vent_nota_cab), 
			...
			'INSERCION/' || usuario || '/' || NOW());
				...
				--AUTOMATISACION DE INSERCION PROCESO CONECTADO
	     if xid_tm = 1 then
	     	FOR d IN SELECT distinct on (id_item) * FROM v_ventas_consolidacion WHERE id_vc = xid_vc LOOP
...
CREATE TABLE public.cobros_tarjetas (
    ...
    estado public.estados,
    auditoria text,
    tar_autorizacion character varying
);
...
CREATE VIEW public.v_serv_evoluciones_foot AS
 SELECT cnc.id_evo,
    ...
    lag(smd_peso.valor) OVER (PARTITION BY cnc.id_cliente ORDER BY cnc.id_evo) AS peso_anterior,
    ...
    p.id_genero,
        ...
        END AS imc,
        CASE
            WHEN ((p.id_genero = 1) AND (smd_altura.valor > (0)::numeric)) THEN ((((1.20 * (smd_peso.valor / (smd_altura.valor ^ (2)::numeric))) + (0.23 * (cnc.evo_edad)::numeric)) - (10.8 * (1)::numeric)) - 5.4)
            WHEN ((p.id_genero = 2) AND (smd_altura.valor > (0)::numeric)) THEN (((1.20 * (smd_peso.valor / (smd_altura.valor ^ (2)::numeric))) + (0.23 * (cnc.evo_edad)::numeric)) - 5.4)
            ELSE NULL::numeric
        END AS grasa_corporal,
        CASE
            WHEN (lag(smd_peso.valor) OVER (PARTITION BY cnc.id_cliente ORDER BY cnc.id_evo) IS NOT NULL) THEN (smd_peso.valor - lag(smd_peso.valor) OVER (PARTITION BY cnc.id_cliente ORDER BY cnc.id_evo))
            ELSE NULL::numeric
        END AS diferencia_peso,
        CASE
            WHEN (lag(smd_peso.valor) OVER (PARTITION BY cnc.id_cliente ORDER BY cnc.id_evo) IS NULL) THEN 'N/A'::text
            WHEN (smd_peso.valor > lag(smd_peso.valor) OVER (PARTITION BY cnc.id_cliente ORDER BY cnc.id_evo)) THEN 'AUMENTÓ'::text
            WHEN (smd_peso.valor < lag(smd_peso.valor) OVER (PARTITION BY cnc.id_cliente ORDER BY cnc.id_evo)) THEN 'DISMINUYÓ'::text
            ELSE 'IGUAL'::text
        END AS estado_peso
   FROM ((((public.serv_evoluciones_cab cnc
...
CREATE VIEW public.v_vent_nota_cab AS
 SELECT cnc.id_not,
    ...
    to_char((t.tim_fecha_fin)::timestamp with time zone, 'DD/MM/YYYY'::text) AS fechatimvigen,
    to_char((cc.vc_fecha)::timestamp with time zone, 'DD/MM/YYYY'::text) AS fecha_original,
    s.suc_nombre,
    s.suc_direccion,
    s.suc_correo,
    s.suc_telefono,
...
21	1	ACTIVO	INSERCION/admin/2025-10-20 13:14:20.447154-03	admin	123	::1	2025-10-20 16:14:20
22	1	ACTIVO	INSERCION/admin/2025-10-20 13:19:22.803632-03	admin	123	::1	2025-10-20 16:19:22
23	1	ACTIVO	INSERCION/admin/2025-10-20 20:35:13.770846-03	admin	123	::1	2025-10-20 23:35:13
24	1	ACTIVO	INSERCION/admin/2025-10-21 13:17:51.793609-03	admin	123	::1	2025-10-21 16:17:51
25	1	ACTIVO	INSERCION/admin/2025-10-21 13:38:53.643898-03	admin	123	::1	2025-10-21 16:38:53
26	2	ACTIVO	INSERCION/juanp/2025-10-21 23:06:34.564775-03	juanp	12345678	::1	2025-10-22 02:06:34
27	1	ACTIVO	INSERCION/admin/2025-10-21 23:19:52.477435-03	admin	12345678	::1	2025-10-22 02:19:52
28	1	ACTIVO	INSERCION/admin/2025-10-22 06:21:14.66869-03	admin	12345678	::1	2025-10-22 09:21:14
29	1	ACTIVO	INSERCION/admin/2025-10-22 08:08:32.08337-03	admin	12345678	::1	2025-10-22 11:08:31
30	1	ACTIVO	INSERCION/admin/2025-10-22 08:18:25.284928-03	admin	12345678	::1	2025-10-22 11:18:25
\.
...
4	1	ACTIVO	INSERCION/admin/2025-10-21 15:54:33.613118-03
\.
...
COPY public.cobros_tarjetas (id_ctar, tar_nro_tarjeta, tar_vencimiento, tar_monto, id_cob, id_ee, id_mt, estado, auditoria, tar_autorizacion) FROM stdin;
\.
...
1	43-32343	2025-10-22	436000	PAGO DE FACTURA	1	1	1	ACTIVO	INSERCION/admin/2025-10-22 11:22:11.554178-03
\.
...
1	2025-10-22	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 08:25:28.197049-03\rCONFIRMACION/admin/2025-10-22 08:26:29.144742-03	n/a
2	2025-10-22	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 08:27:21.911528-03\rCONFIRMACION/admin/2025-10-22 08:28:24.338231-03	n/a
3	2025-10-22	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 08:28:40.57028-03\rCONFIRMACION/admin/2025-10-22 08:29:06.9679-03	s
\.
...
1	5	6	3	7000	INSERCION/admin/2025-10-22 08:25:56.207749-03	1
1	2	9	4	250000	INSERCION/admin/2025-10-22 08:26:19.902332-03	1
2	6	13	2	10000	INSERCION/admin/2025-10-22 08:28:00.750571-03	1
3	6	6	1	10000	INSERCION/admin/2025-10-22 08:28:53.892285-03	1
\.
...
1	2025-10-22	1210000	1210000	ACTIVO	INSERCION/admin/2025-10-22 08:23:20.044903-03
\.
...
1	2025-10-22	2025-10-22	DEBITO	1	1	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 09:24:54.995349-03\rCONFIRMACION/admin/2025-10-22 09:25:01.057165-03	11	N/A	N/A	002-005-1234568
\.
...
1	2025-10-22	2025-10-22	2025-10-22	1	1	1	2	1	2	1	3	n/a	CONFIRMADO	INSERCION/admin/2025-10-22 08:29:44.658225-03\rENVIADO/admin/2025-10-22 08:30:45.044748-03\rCONFIRMACION/admin/2025-10-22 08:30:54.616195-03
\.
...
1	6	4	INSERCION/admin/2025-10-22 08:30:08.431776-03	2
1	1	3	INSERCION/admin/2025-10-22 08:30:29.042229-03	2
\.
...
1	2025-10-22	CONTADO	0	1	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 08:23:04.485507-03\rCONFIRMACION/admin/2025-10-22 08:23:20.044903-03	0	012-003-0015345	42342342	2025-10-22
\.
...
1	2025-10-22	CONTADO	0	1	1	1	CERRADO	INSERCION/admin/2025-10-22 08:22:22.527344-03\rCONFIRMACION/admin/2025-10-22 08:22:35.144561-03\rCERRAR/admin/2025-10-22 08:23:20.044903-03	0
\.
...
1	2025-10-22	2025-10-22	1	1	CERRADO	INSERCION/admin/2025-10-22 08:20:22.712271-03\rCONFIRMACION/admin/2025-10-22 08:20:57.033449-03\rCERRAR/admin/2025-10-22 08:22:07.105425-03
\.
...
1	2025-10-22	2025-10-22	909	n/a	1	1	1	CERRADO	INSERCION/admin/2025-10-22 08:21:38.378266-03\rCONFIRMACION/admin/2025-10-22 08:22:07.105425-03\rCERRAR/admin/2025-10-22 08:22:35.144561-03
\.
...
1	1	300000	2025-10-23	12345678	1	PAGADO	INSERCION/admin/2025-10-22 08:39:55.413953-03	4
\.
...
1	0	109997	0	ACTIVO	INSERCION/admin/2025-10-22 08:23:20.044903-03
\.
...
1	0	130544	0	ACTIVO	INSERCION/admin/2025-10-22 08:56:46.0131-03
2	0	0	0	ACTIVO	INSERCION/admin/2025-10-22 08:57:26.848494-03
\.
...
7	PRODUCTO DAÑADO O VENCIDO	NEGATIVO	ACTIVO
\.
...
2	7	1	ACTIVO	INSERCION/admin/2023-10-16 17:06:59.625562-03
\.
...
2	JUAN	PEREZ	1234563-4	1234563	XXX-4321	matiasmarfe@gmail.com	1996-09-02	0981432764	t	ACTIVO	INSERCION/admin/2023-10-31 16:59:03.174228-03	1	2	1	37
10	SDFAAASDF	FASFA	6321987-0\t	535345345	RWEREWRTWER	WTWETWE	2025-08-30		t	ACTIVO	INSERCION/admin/2025-08-30 07:15:41.206513-04	1	1	1	28
12	IVAN	TORREZ	34534535-9	12345678	YYYY323X	ivantorrez@gmail.com	2006-02-17		t	ACTIVO	INSERCION/admin/2025-10-03 21:56:55.242095-04	1	1	1	24
...
1	2025-10-22	2025-10-31	Perdida de peso	LUNES A VIERNES	n7a	\N	4	\N	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 08:51:40.41609-03\rCONFIRMACION/admin/2025-10-22 08:52:11.943165-03
\.
...
1	11	20	280	12	35	INSERCION DETALLE/admin/2025-10-22 08:52:05.706151-03	1
\.
...
1	2025-10-22 08:41:00	2025-10-22 08:41:17	4	1	1	1	SALIDA	INSERCION/admin/2025-10-22 08:41:01.563128-03\rSALIDA/admin/2025-10-22 08:41:18.237265-03
\.
...
1	\N	\N	\N	\N	\N	4	4	1	\N	\N	\N	\N
2	2025-10-22	n/a	24	21.30	14.88	4	4	1	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 09:13:19.388749-03\rCONFIRMACION/admin/2025-10-22 09:13:42.656665-03
3	2025-10-22	n/a	24	21.30	14.88	4	4	1	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 09:14:06.5622-03\rCONFIRMACION/admin/2025-10-22 10:47:42.027834-03
4	2025-10-22	n/A	40	27.68	26.22	2	4	2	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 10:55:39.64256-03\rCONFIRMACION/admin/2025-10-22 10:56:05.039015-03
5	2025-10-22	m	40	27.68	26.22	2	4	2	1	1	PENDIENTE	INSERCION/admin/2025-10-22 10:56:15.37134-03
\.
...
2	4	1.80	INSERCION DETALLE/admin/2025-10-22 09:13:19.388749-03
2	1	80	INSERCION DETALLE/admin/2025-10-22 09:13:33.814584-03
3	4	1.80	INSERCION DETALLE/admin/2025-10-22 09:14:06.5622-03
3	1	90	INSERCION DETALLE/admin/2025-10-22 09:14:17.518902-03
4	4	1.70	INSERCION DETALLE/admin/2025-10-22 10:55:39.64256-03
4	1	78	INSERCION DETALLE/admin/2025-10-22 10:56:00.842467-03
5	4	1.70	INSERCION DETALLE/admin/2025-10-22 10:56:15.37134-03
5	1	77	INSERCION DETALLE/admin/2025-10-22 10:56:25.913894-03
\.
...
1	2025-10-22	24	n/a	4	4	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 08:46:12.169577-03\rCONFIRMACION/admin/2025-10-22 09:08:52.761382-03
2	2025-10-22	40	n/a	2	4	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 10:54:38.618371-03\rCONFIRMACION/admin/2025-10-22 10:55:25.710825-03
\.
...
1	4	1.80	INSERCION DETALLE/admin/2025-10-22 08:46:22.067016-03 | TRIGGER UPDATE: 2025-10-22 09:13:19.388749-03 | TRIGGER UPDATE: 2025-10-22 09:14:06.5622-03
1	1	90	INSERCION DETALLE/admin/2025-10-22 08:46:32.044745-03 | TRIGGER UPDATE: 2025-10-22 09:13:33.814584-03 | TRIGGER UPDATE: 2025-10-22 09:14:17.518902-03
2	4	1.70	INSERCION DETALLE/admin/2025-10-22 10:55:01.522326-03 | TRIGGER UPDATE: 2025-10-22 10:55:39.64256-03 | TRIGGER UPDATE: 2025-10-22 10:56:15.37134-03
2	1	77	INSERCION DETALLE/admin/2025-10-22 10:54:51.239725-03 | TRIGGER UPDATE: 2025-10-22 10:56:00.842467-03 | TRIGGER UPDATE: 2025-10-22 10:56:25.913894-03
\.
...
1	2025-10-22	2025-11-08	N/A	4	1	1	CONFIRMADO	INSERCION/admin/2025-10-22 08:36:53.381405-03\rCONFIRMACION/admin/2025-10-22 08:39:55.413953-03
\.
...
1	2025-10-22	n/a	4	4	1	1	PENDIENTE	INSERCION/admin/2025-10-22 09:12:25.205134-03
\.
...
1	2025-10-22	24	n/A	\N	4	4	1	1	PENDIENTE	INSERCION/admin/2025-10-22 08:43:50.573018-03
\.
...
1	8	4	15	60	INSERCION DETALLE/admin/2025-10-22 08:44:16.158653-03	Press fances
1	5	2	20	70	INSERCION DETALLE/admin/2025-10-22 08:45:39.478379-03	SENTADILLA
\.
...
1	2025-10-22	2025-10-22	1	1	4	CONFIRMADO	INSERCION/admin/2025-10-22 08:35:22.925454-03\rCONFIRMACION/admin/2025-10-22 08:36:31.118649-03	19	70	1.80	t
2	2025-10-22	2025-10-22	1	1	2	CONFIRMADO	INSERCION/admin/2025-10-22 10:54:14.196332-03\rCONFIRMACION/admin/2025-10-22 10:54:22.39855-03	32	80	170	f
\.
...
6	23456789	FACTURA	2024-04-01	2024-12-31	4	4000	3	2	ACTIVO	\N
\.
...
2	juanp	25d55ad283aa400af464c76d713c07ad	/tesis/imagenes/usuarios/2.png	ACTIVO	\N	2	2	2
1	admin	25d55ad283aa400af464c76d713c07ad	/tesis/imagenes/usuarios/1.png	ACTIVO	\N	1	1	1
\.
...
1	1	1	1	2025-10-22 08:55:19	\N	50000	50000	450000	100000	600000	ABIERTO	INSERCION/admin/2025-10-22 08:55:33.552118-03	50000
\.
...
1	1	1	1	1	2025-10-22 14:19:21	PENDIENTE	INSERCION/admin/2025-10-22 11:21:26.138548-03	1
\.
...
1	1	3	1000000	INSERCION/admin/2025-10-22 11:21:31.527183-03	1436000
\.
...
2	2	300000	0	PAGADO	INSERCION/admin/2025-10-22 08:57:26.848494-03	1	2025-10-22
1	1	1436000	0	PENDIENTE	INSERCION/admin/2025-10-22 08:56:46.0131-03	1	2025-10-22
\.
...
1	2025-10-22 10:50:53.444192	2025-10-22 00:00:00	CREDITO	a	1	1	1	4	PENDIENTE	INSERCION/admin/2025-10-22 10:50:53.444192-03	5	4	002-002-0000003	\N	\N
\.
...
1	2025-10-22	CONTADO	1	0	003-002-0000002	6	1	1	4	CONFIRMADO	INSERCION/admin/2025-10-22 08:56:09.847535-03\rCONFIRMACION/admin/2025-10-22 08:56:46.0131-03	4
2	2025-10-22	CONTADO	1	0	003-002-0000003	6	1	1	4	CONFIRMADO	INSERCION/admin/2025-10-22 08:57:19.552074-03\rCONFIRMACION/admin/2025-10-22 08:57:26.848494-03	3
\.
...
1	2025-10-22	2025-10-22	n/a	1	1	4	CERRADO	INSERCION/admin/2025-10-22 08:54:50.700317-03\rCONFIRMACION/admin/2025-10-22 08:55:12.644107-03\rCERRAR/admin/2025-10-22 08:56:46.0131-03
\.
